...
线上部署 `27681ca7b1` 后，从网页提交强制回退到 `dbbe36cc3a`，后台在替换前拒绝，提示配置已变化。连续读取 Docker 检查结果发现，挂载数组顺序会变化，实际来源、目标与权限一致；这既使报告在通过与未验证间波动，也使确认状态不稳定。修复将 Compose 对象键以及服务挂载、端口集合按稳定顺序比较，保留启动参数等有序列表。新增回归先复现失败，再验证确认后仅列表重排仍能执行，而来源、权限与命令变化仍改变检查状态。故障发生时业务容器没有切换，数据库与数据挂载保持不变

### 2026-09-21 回退与签名线上验收结果

最终业务镜像与独立 release-worker 均为 `d4b774ce2645f31ff77721335616b3b9245df906`，GitHub 构建 `35570987498` 成功，包含生产前端构建。修复挂载顺序后，回退测试与 Docker 边界测试共 46 项通过，相关源码 basedpyright 为 0 错误、0 警告，Ruff 检查与格式通过。本次未重跑整个项目测试；前述 Manager、网关和 UI 数字属于构建前对应工作树的记录

通过 SSH 通道登录生产管理页面，验证回退报告的分类卡片、滚动正文、固定确认区、可选版本、强制输入与二次确认。普通回退实际执行 `27681ca7b1` 到 `dbbe36cc3a`，任务 `c667e9677f32f030239e75ec7def893f` 成功且两个业务容器健康，再部署回最终 `d4b774ce26`，任务 `8ec5ca2737ca4a10600eb3a5cebeef84` 成功。前后两个数据库容器 ID、各服务环境变量和挂载集合一致，没有恢复数据快照。最终版本备份 ID 为 `94c0b8106b86b50a9c36f37e`，启动 Compose 镜像引用已同步，网页备注已保存。更早的 `663d65e301` 存在号池状态、凭据与日志定义差异，只验证强制确认与取消，没有在生产运行它；不能把普通回退成功表述为真实不兼容版本的强制运行已验收

对 `27681ca7b1` 的公网临时虚拟 Key 测试中，模型列表正常，Sol 普通调用、续聊和流式成功，成功请求约 4.4～24 秒。两次含人工损坏思考块的请求成功，但日志没有记录网关签名恢复，不能认定触发过拒绝后重试。完整日志证实请求仍含该块。加密压缩上下文请求返回中文 `encrypted_compaction:input[0]` 说明，未删除该上下文冒充恢复成功。Astra 的普通测试返回 `invalid_prompt`；Sol 工具历史测试遇到 502，未完成。标准费用表中 Sol 测试 7 条记录合计 0.0074 美元，完整日志 5 条，测试密钥已删除；实际带密文的跨模型续聊没有完成验证

最终 `d4b774ce26` 上，仪表盘、卡片、版本管理、日常日志、完整日志接口均返回 200。真实新密文补测两次因 Sol 502 未生成可用响应，最后 Sol 与 Terra 的普通请求也返回 502。同期 CLIProxyAPI 与网关日志出现 `server_is_overloaded`，容器无 OOM 且资源占用低；没有为绕开这类错误修改账号或上游设置。签名修复已部署，线上验收只部分通过，不能承诺 Key 调用稳定或真实跨站问题完全解决。用户明确自行测试 Codex 外站旧会话，不再要求用户为本轮提供会话正文或密钥

服务器验收报告位于 `/opt/litellm-releases/acceptance-27681ca7b1/`，部署前配置含凭据，仅保存在私有目录，不得复制进仓库或对话。浏览器页面在更新前已打开时，需重新加载文档确认使用新资源；仅刷新列表可能仍运行旧前端模块。公网 UI 另有 Basic 登录，网页验收通过已认证 SSH 本地通道完成，未修改公网访问控制
