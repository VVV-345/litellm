...
| 当前号池 CLIProxyAPI | v7.2.155，commit `7fac6b15` | 已与 Cockpit 内嵌版本对齐 |
...
| 删除账号 | 摘除路由、停止实例、删除数据和元数据 | 已有清理检查点和删除流程；环境、卡片 Key、策略、租约、会话绑定和冷却状态会一并清理 | 增加删除前卡片 Key 引用提示和更完整的审计展示 |
...
| 批量操作 | 批量刷新额度、授权、启停、冷却、删除、修改路由策略 | 已有持久化异步刷新、重新授权、启用、停用、冷却、解除冷却、删除和策略任务；授权链接、SSH 命令、设备码及过期时间随任务项持久化，删除需要二次确认，任务按目标版本和幂等操作 ID 执行 | 后续补充批量任务筛选和历史清理策略 |
...
| `duration_ms`、`routing_reason`、`cost_usd`、`final_status` | 处理耗时、实际选择原因、上游明确报告的美元成本和最终成功、失败、冷却或取消结果；金额缺失时保持为空 |
...
- 增加批量刷新配额、批量重新授权、批量冷却、批量启停和批量策略更新
