...
- 建立 Cockpit 全部设置的供应商能力矩阵
- 将通用策略字段和供应商扩展字段保存到账号策略中
- 在对应 CLIProxyAPI 或代理适配层执行每个供应商支持的身份、指纹、客户端、连接和协议设置
- Codex 先投影已核实的 `off`、`device`、`session`、`full`，其他供应商按独立核实结果接入
- 单独实现客户端准入字段，不把它和身份或指纹策略混淆
- 增加配置版本和 CLIProxyAPI 能力探测

验收：每个 Cockpit 设置都有明确的适用范围和执行位置；关闭策略时请求保持原样；启用策略时只改写该供应商已核实的字段；不支持的账号类型或供应商不会误套用；跨账号切换时映射不会泄露原账号会话关系
...
先完成账号卡片、配额、状态、模型和配置页面，再建立 Virtual Key 到账号范围的授权模型，然后实现代理层候选过滤、优先级、会话粘性、冷却和故障切换。之后接入各供应商的请求、身份和协议策略，再完成 Compact 数据面，最后再接本机 Codex、Cursor 多开
