...
| --- | --- | --- | --- |
| Dashboard | 总账号数、可用数、冷却数、错误数、额度和请求概况 | 已有账号、可用和待授权汇总，以及请求、失败、Token 和平均耗时统计 | 后续补充冷却汇总、路由健康和更完整的配额概况 |
...
| 错误日志 | 按渠道、账号、请求、时间、阶段和错误原因查询故障 | 已有独立结构化日志、筛选、详情、同请求尝试链、脱敏和请求统计 | 真实数据库负载、保留清理和大规模查询仍需部署验证 |
| 标签和分组 | 标签、分组、筛选、批量选择 | 已有标签、分组搜索和批量选择 | 后续可补供应商分组快捷筛选 |
| 批量操作 | 批量刷新额度、授权、启停、冷却、删除、修改路由策略 | 已有持久化异步刷新、启用、停用、冷却、解除冷却和策略任务，页面已接刷新、启停和冷却操作 | 批量授权和删除未实现；批量策略已有服务端能力，页面编辑入口未接 |
| API Service | 启停服务、Base URL、端口、服务 Key、健康状态 | 当前服务由 LiteLLM 和 Manager 配置控制 | 页面展示服务状态；公共入口只保留 LiteLLM，不向每个账号发布宿主机端口 |
| API Key 管理 | Key 名称、账号范围、优先账号、模型权限、Token 限额、状态 | 已有每卡片独立 `cpk_` Key、一次性明文、复制、撤销、重置、状态和最近使用时间 | Token 限额、速率限制和下游用户权限继续由 NewAPI 管理 |
| 路由策略 | 自动、随机、单账号、配额、套餐、到期、自定义 | 已有卡片范围过滤、自动、随机、优先级、配额、优先账号和备用账号；套餐、到期返回明确不支持 | 取得可信套餐和订阅到期元数据后再启用对应排序 |
...
| 故障处理 | 重试、冷却、备用账号、错误分类和故障切换 | 已有有限重试、连接失败与可重试状态切号、401/403/429 临时冷却、并发回退和流式中断保护 | 不重放带 `previous_response_id` 的有状态请求；真实上游边界仍需 E2E |
| 高级传输 | WebSocket、图片策略、超时、调试日志 | 已有图片入口准入和请求超时；卡片 Key WebSocket 明确拒绝；调试开关仅保存 | WebSocket 和调试日志运行时未实现 |
| 统计 | 账号、模型、Key 维度的请求数、Token、延迟、成本和错误 | 已有卡片、账号和模型筛选的请求数、成功、失败、重试、Token 和平均耗时 | 成本和显式路由原因字段尚未实现；卡片 Key ID 已写入事件，可通过日志查询 |
...
| WebSocket | `websocket_enabled`、超时和 Origin 策略 | 按供应商默认 | 账号策略表 | 当前卡片 Key WebSocket 统一返回不支持，保存值不改变运行时 |
| 调试 | `debug_log_enabled`、保留时间、敏感字段脱敏 | 关闭 | 账号策略表 | 当前仅保存；结构化请求日志始终脱敏且不受此开关控制 |
...
错误日志必须独立于账号当前状态。账号卡片上的 `last_error` 只保留最近一次可展示摘要，结构化错误日志保存完整的脱敏事件，用于排查历史问题。每条日志至少要能回答：哪个渠道、哪张卡片、哪个上游账号、哪个卡片 Key、哪个模型、哪个请求、在什么时间、处于哪个处理阶段、发生了什么错误、是否重试、是否切换了账号以及最终结果是什么
...
| `operation`、`stage` | 操作类型和处理阶段，例如授权、配额刷新、配置同步、入口鉴权、路由、连接、上游请求、响应解析、容器生命周期 |
| `model`、`endpoint`、`method` | 模型和请求接口元数据 |
| `error_category`、`severity` | 稳定错误分类和严重级别，例如认证、额度、限流、网络、超时、模型不可用、协议、配置、容器或内部错误 |
| `http_status`、`upstream_code` | HTTP 状态码和上游错误码 |
| `retryable`、`retry_count`、`switched_account`、`next_account_id` | 是否可重试、重试次数、是否切号以及切换到的账号 |
| `message`、`detail` | 脱敏后的简短错误和排查详情 |
| `duration_ms`、`final_status` | 处理耗时和最终成功、失败、冷却或取消结果 |

日志来源至少包括：OAuth 授权失败、模型发现失败、配额读取失败、配置渲染或同步失败、容器启动或健康检查失败、卡片 Key 无效、模型无权限、账号冷却、上游 401/403/429/5xx、连接超时、流式中断、Compact 不支持、重试失败和自动切号

页面需要提供全局日志页、卡片最近错误、账号最近错误和请求详情。查询条件至少包括时间范围、渠道、供应商、卡片、账号、卡片 Key、模型、错误分类、处理阶段、是否重试和是否切号。日志详情要能从一次请求看到所有尝试的顺序

日志必须脱敏。不能写入 OAuth token、refresh token、卡片 Key 明文、代理密码、完整 Authorization Header、完整请求体或完整响应体。需要保留排查上下文时，只保存请求 ID、模型、状态码、上游错误码、经过截断和脱敏的错误消息以及必要的摘要哈希。日志表需要按时间和卡片、账号、请求 ID 建索引，并提供保留期限和清理任务
...
| `GET` | `/account_pool/logs` | 按时间、渠道、供应商、卡片、账号、Key、模型、阶段和错误分类查询日志 |
| `GET` | `/account_pool/logs/{event_id}` | 查看单条错误事件及同一请求的所有尝试，不返回敏感字段 |
...
- 增加全局日志页、卡片最近错误、账号最近错误、时间和渠道筛选、错误详情和同请求尝试链
...
| 日志 | 渠道、供应商、卡片、账号、Key、请求、时间、阶段、错误分类、上游错误码、重试、切号、查询和脱敏 |
...
- 每次失败都能定位到渠道、卡片、账号、时间、处理阶段、错误原因、重试和切号结果
- 日志详情不会泄露卡片 Key、OAuth 凭据、代理密码、完整请求体或完整响应体
