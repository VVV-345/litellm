...
| 高级传输 | WebSocket、图片策略、超时、调试日志 | 已有图片入口准入、请求超时、卡片 Key WebSocket 双向转发和卡片级脱敏调试详情 | 真实供应商 WebSocket、Origin 和大消息边界仍需 E2E |
...
| 调试 | `debug_log_enabled`、保留时间、敏感字段脱敏 | 关闭 | 账号策略表 | HTTP 和 WebSocket 请求按卡片开关记录字段名、路由账号和传输类型，敏感值不落日志，已实现 |
