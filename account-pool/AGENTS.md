...
- Environment operations live in `application/environments/` and receive explicit typed dependencies from `EnvironmentService`. Move complete operations with their lock and compensation scopes; keep the shared lock registry and client lifetime at the existing owner. Check extracted method names for collisions before replacing private names
...
- Quota contracts live in `providers/usage/contracts.py`, response parsers in `providers/usage/`, and routing quota calculations in `application/quota_state.py`. Internal consumers use these modules directly; `quota.py` preserves existing imports, including domain types previously re-exported through imports
- Scope structural cleanup to account-pool code. Do not refactor native LiteLLM modules as part of this work unless the user explicitly expands the scope
