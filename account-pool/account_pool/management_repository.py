    ...
    async def stats(
        ...
        async with database_connection(self._database_url) as connection:
            summary_cursor: Final = await connection.execute(
                sql.SQL(
                    "WITH attempts AS (SELECT * FROM account_pool_error_log WHERE {}), final_requests AS ("
                    "SELECT DISTINCT ON (request_id, card_id) payload FROM attempts "
                    ...
                    "(SELECT coalesce(sum((payload->>'cache_read_input_tokens')::bigint), 0) FROM attempts) AS cache_read_input_tokens, "
                    "(SELECT coalesce(sum((payload->>'cache_creation_input_tokens')::bigint), 0) FROM attempts) AS cache_creation_input_tokens, "
                    "(SELECT count(*) FROM (SELECT request_id, card_id FROM attempts GROUP BY request_id, card_id "
                    "HAVING bool_and(payload->>'cost_usd' IS NOT NULL)) known) AS known_cost_requests, "
                    "(SELECT sum((payload->>'cost_usd')::double precision) FROM attempts) AS total_cost_usd, "
