  ...
  if (!canViewToolPolicies) {
    return (
      <div className="p-6 w-full min-w-0 flex-1">
        <h1 className="text-2xl font-semibold text-foreground mb-2">{t("ui.Tool Policies")}</h1>
        <p className="text-sm text-muted-foreground">{t("ui.Tool Policies is only available to admin users.")}</p>
      </div>
    );
  }
