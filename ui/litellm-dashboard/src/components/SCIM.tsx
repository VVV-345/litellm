...
const SCIMConfig: React.FC<SCIMConfigProps> = ({ accessToken, userID, proxySettings }) => {
  ...
  const handleCreateSCIMToken = async (values: any) => {
    if (!accessToken || !userID) {
      NotificationsManager.fromBackend(t("ui.You need to be logged in to create a SCIM token"));
      return;
    }
    ...
    try {
      ...
      NotificationsManager.success(t("ui.SCIM token created successfully"));
    } catch (error: any) {
      console.error("Error creating SCIM token:", error);
      NotificationsManager.fromBackend(t("ui.Failed to create SCIM token:") + " " + parseErrorMessage(error));
    } finally {
    ...
    <Grid numItems={1}>
      <Card>
        <div className="flex items-center mb-4">
          <Title>{t("ui.SCIM Configuration")}</Title>
        </div>
        <Text className="text-gray-600">
          {t(
            "ui.System for Cross-domain Identity Management (SCIM) allows you to automatically provision and manage users and groups in LiteLLM.",
          )}
        </Text>
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <Title className="text-lg flex items-center">
                ...
                {t("ui.SCIM Tenant URL")}
              </Title>
            </div>
            <Text className="text-gray-600 mb-3">
              {t("ui.Use this URL in your identity provider SCIM integration settings.")}
            </Text>
            <div className="flex items-center">
              <TextInput value={scimBaseUrl} disabled={true} className="grow" />
              <CopyToClipboard
                text={scimBaseUrl}
                onCopy={() => NotificationsManager.success(t("ui.URL copied to clipboard"))}
              >
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <Title className="text-lg flex items-center">
                ...
                {t("ui.Authentication Token")}
              </Title>
            </div>

            <Callout title={t("ui.Using SCIM")} color="blue" className="mb-4">
              {t(
                "ui.You need a SCIM token to authenticate with the SCIM API. Create one below and use it in your SCIM provider configuration.",
              )}
            </Callout>
            ...
            {!tokenData ? (
              <div className="bg-gray-50 p-4 rounded-lg">
                <Form form={form} onFinish={handleCreateSCIMToken} layout="vertical">
                  <Form.Item
                    name="key_alias"
                    label={t("ui.Token Name")}
                    rules={[{ required: true, message: t("ui.Please enter a name for your token") }]}
                  >
                    <TextInput placeholder={t("ui.SCIM Access Token")} />
                  </Form.Item>
                  <Form.Item>
                    <TremorButton
                      variant="primary"
              ...
              <Card className="border border-yellow-300 bg-yellow-50">
                <div className="flex items-center mb-2 text-yellow-800">
                  ...
                  <Title className="text-lg text-yellow-800">{t("ui.Your SCIM Token")}</Title>
                </div>
                <Text className="text-yellow-800 mb-4 font-medium">
                  {t("ui.Make sure to copy this token now. You will not be able to see it again.")}
                </Text>
                <div className="flex items-center">
                  <TextInput value={tokenData.key} className="grow mr-2 bg-white" type="password" disabled={true} />
                  <CopyToClipboard
                    text={tokenData.key}
                    onCopy={() => NotificationsManager.success(t("ui.Token copied to clipboard"))}
                  >
