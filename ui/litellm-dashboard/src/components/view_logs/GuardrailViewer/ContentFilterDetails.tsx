...
const ContentFilterDetails: React.FC<ContentFilterDetailsProps> = ({ response }) => {
      ...
      return (
        <div className="bg-white rounded-lg border border-red-200 p-4">
          <div className="text-red-800">
            <h5 className="font-medium mb-2">{t("ui.Error")}</h5>
  ...
  if (detections.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <div className="text-gray-600 text-sm">{t("ui.No detections found")}</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Patterns Section */}
      {patterns.length > 0 && (
        <Section title={t("ui.Patterns Matched")} count={patterns.length} defaultOpen={true}>
          <div className="space-y-2">
            {patterns.map((detection, idx) => (
              <div key={idx} className="p-3 bg-gray-50 rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("ui.Pattern:")}>{detection.pattern_name || "unknown"}</KV>
                  </div>
                  <div className="space-y-1">
                    <KV label={t("ui.Action:")}>{chip(detection.action, detection.action === "BLOCK" ? "red" : "blue")}</KV>
                  </div>
                </div>
              </div>
            ))}
      ...
      {blockedWords.length > 0 && (
        <Section title={t("ui.Blocked Words Detected")} count={blockedWords.length} defaultOpen={true}>
          <div className="space-y-2">
            {blockedWords.map((detection, idx) => (
              <div key={idx} className="p-3 bg-gray-50 rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("ui.Keyword:")} mono>
                      {detection.keyword || "unknown"}
                    </KV>
                    {detection.description && <KV label={t("ui.Description:")}>{detection.description}</KV>}
                  </div>
                  <div className="space-y-1">
                    <KV label={t("ui.Action:")}>{chip(detection.action, detection.action === "BLOCK" ? "red" : "blue")}</KV>
                  </div>
      ...
      {categoryKeywords.length > 0 && (
        <Section title={t("ui.Category Keywords Detected")} count={categoryKeywords.length} defaultOpen={true}>
          <div className="space-y-2">
            {categoryKeywords.map((detection, idx) => (
              <div key={idx} className="p-3 bg-gray-50 rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("ui.Category:")}>{detection.category || "unknown"}</KV>
                    <KV label={t("ui.Keyword:")} mono>
                      {detection.keyword || "unknown"}
                    </KV>
                    {detection.severity && (
                      <KV label={t("ui.Severity:")}>
                        {chip(
                          detection.severity,
      ...
      <Section title={t("ui.Raw Detection Data")} defaultOpen={false}>
