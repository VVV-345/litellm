  ...
  return (
    <div className="mt-4">
      <div className="flex items-center mb-2 cursor-pointer" onClick={() => setEntityListExpanded(!entityListExpanded)}>
        ...
        <h4 className="font-medium">{t("ui.Detected Entities")} ({entities.length})</h4>
      </div>
              ...
              <div key={index} className="border rounded-lg overflow-hidden">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className={`font-mono ${getScoreColor(entity.score)}`}>{t("ui.Score:")} {entity.score.toFixed(2)}</span>
                  </div>
                  <span className="text-xs text-gray-500">
                    {t("ui.Position:")} {entity.start}-{entity.end}
                  </span>
                </div>
                ...
                {isExpanded && (
                  <div className="p-3 border-t bg-white">
                    <div className="grid grid-cols-2 gap-4 mb-2">
                      <div className="space-y-2">
                        <div className="flex">
                          <span className="font-medium w-1/3">{t("ui.Entity Type:")}</span>
                          <span>{entity.entity_type}</span>
                        </div>
                        <div className="flex">
                          <span className="font-medium w-1/3">{t("ui.Position:")}</span>
                          <span>
                            {t("ui.Characters")} {entity.start}-{entity.end}
                          </span>
                        </div>
                        <div className="flex">
                          <span className="font-medium w-1/3">{t("ui.Confidence:")}</span>
                      ...
                      <div className="space-y-2">
                        {entity.recognition_metadata && (
                          <>
                            <div className="flex">
                              <span className="font-medium w-1/3">{t("ui.Recognizer:")}</span>
                              <span>{entity.recognition_metadata.recognizer_name}</span>
                            </div>
                            <div className="flex overflow-hidden">
                              <span className="font-medium w-1/3">{t("ui.Identifier:")}</span>
                        ...
                        {entity.analysis_explanation && (
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("ui.Explanation:")}</span>
