...
const CloudZeroExportModal: React.FC<CloudZeroExportModalProps> = ({ isOpen, onClose, accessToken }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        NotificationsManager.fromBackend(
          t("ui.Failed to load existing settings: {{error}}", {
            error: errorData.error || t("ui.Unknown error"),
          }),
        );
      }
    } catch (error) {
      console.error("Error loading CloudZero settings:", error);
      NotificationsManager.fromBackend(t("ui.Failed to load existing settings"));
    } finally {
  ...
  const handleSaveCloudZeroSettings = async (values: CloudZeroSettings) => {
    ...
    try {
      ...
      if (response.ok) {
        NotificationsManager.success(data.message || t("ui.CloudZero settings saved successfully"));
        setExistingSettings({
        ...
        NotificationsManager.fromBackend(data.error || t("ui.Failed to save CloudZero settings"));
        return false;
      }
    } catch (error) {
      console.error("Error saving CloudZero settings:", error);
      NotificationsManager.fromBackend(t("ui.Failed to save CloudZero settings"));
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCloudZero = async () => {
    ...
    try {
      ...
      if (response.ok) {
        NotificationsManager.success(data.message || t("ui.Export to CloudZero completed successfully"));
        onClose();
      } else {
        NotificationsManager.fromBackend(data.error || t("ui.Failed to export to CloudZero"));
      }
    } catch (error) {
      console.error("Error exporting to CloudZero:", error);
      NotificationsManager.fromBackend(t("ui.Failed to export to CloudZero"));
    } finally {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      NotificationsManager.info(t("ui.CSV export functionality coming soon!"));
      onClose();
    } catch (error) {
      console.error("Error exporting CSV:", error);
      NotificationsManager.fromBackend(t("ui.Failed to export CSV"));
    } finally {
      setExportLoading(false);
    }
  };
  ...
  const exportOptions = [
    {
      ...
      label: (
        <div className="flex items-center gap-2">
          ...
          />
          <span>{t("ui.Export to CloudZero")}</span>
        </div>
      ),
    },
    {
  ...
  return (
    <Modal title={t("ui.Export Data")} open={isOpen} onCancel={handleModalClose} footer={null} width={600} destroyOnHidden>
      <div className="space-y-4">
        {/* Export Type Selection */}
        <div>
          <Text className="font-medium mb-2 block">{t("ui.Export Destination")}</Text>
          ...
          <div>
            {settingsLoading ? (
              ...
              <>
                {existingSettings && (
                  <Callout
                    title={t("ui.Existing CloudZero Configuration")}
                    icon={() => (
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                          strokeLinecap="round"
                  ...
                  >
                    <Text>
                      {t("ui.API Key: {{key}}", { key: existingSettings.api_key_masked })}
                      <br />
                      {t("ui.Connection ID: {{id}}", { id: existingSettings.connection_id })}
                    </Text>
                  </Callout>
                )}
                ...
                {!existingSettings && (
                  <Form form={form} layout="vertical">
                    ...
                    >
                      <TextInput type="password" placeholder={t("ui.Enter your CloudZero API key")} />
                    </Form.Item>

                    <Form.Item
                      label={t("ui.Connection ID")}
                      name="connection_id"
                      rules={[{ required: true, message: t("ui.Please enter the CloudZero connection ID") }]}
                    >
                      <TextInput placeholder={t("ui.Enter CloudZero connection ID")} />
                    </Form.Item>
                  </Form>
                )}
              </>
        ...
        {exportType === "csv" && (
          <Callout
            ...
            color="blue"
          >
            <Text>{t("ui.Export your usage data as a CSV file for analysis in spreadsheet applications.")}</Text>
          </Callout>
        )}
        ...
        <div className="flex justify-end space-x-2 pt-4">
          ...
          <Button onClick={handleExport} loading={loading || exportLoading} disabled={loading || exportLoading}>
            {exportType === "cloudzero" ? t("ui.Export to CloudZero") : t("ui.Export CSV")}
          </Button>
        </div>
      </div>
    </Modal>
