...
export const CreateUserButton: React.FC<CreateuserProps> = ({
  ...
  if (isEmbedded) {
    return (
      ...
      >
        <Alert
          message={t("ui.Email invitations")}
          description={
            <>
              {t("ui.New users receive an email invite only when an email integration (SMTP, Resend, or SendGrid) is configured.")}{" "}
          ...
          className="mb-4"
        />
        <Form.Item label={t("ui.User Email")} name="user_email">
          <TextInput placeholder="" />
        </Form.Item>
        <Form.Item label={t("ui.User Role")} name="user_role">
          <Select2>
            {possibleUIRoles &&
        ...
        <Form.Item label={t("ui.Team")} name="team_id">
          <TeamDropdown />
        </Form.Item>

        <Form.Item label={t("ui.Metadata")} name="metadata">
          <Input.TextArea rows={4} placeholder={t("ui.Enter metadata as JSON")} />
        </Form.Item>

        <Form.Item label={t("ui.Send invitation email")} name="send_invite_email" valuePropName="checked">
          <Checkbox />
        </Form.Item>

        <div style={{ textAlign: "right", marginTop: "10px" }}>
          <Button htmlType="submit">{t("ui.Create User")}</Button>
        </div>
      </Form>
    );
  }
    ...
    <div className="flex gap-2">
      ...
      >
        <Space direction="vertical" size="middle">
          <Text className="mb-1">{t("ui.Create a User who can own keys")}</Text>
          <Alert
            message={t("ui.Email invitations")}
            description={
              <>
        ...
        >
          ...
          <Form.Item
            label={
              <span>
                ...
                <Tooltip title={t("ui.This role is independent of any team/org specific roles. Configure Team / Organization Admins in the Settings")}>
                  <InfoCircleOutlined />
                </Tooltip>
              </span>
            }
          ...
          <Form.Item
            ...
            className="gap-2"
            name="team_id"
            help={t("ui.If selected, user will be added as a 'user' role to the team.")}
          >
            <TeamDropdown />
          </Form.Item>
          ...
          <Form.Item
            ...
            help={t("ui.The user will be added to the selected organization(s).")}
          >
            <Select mode="multiple" placeholder={t("ui.Select Organization")} style={{ width: "100%" }}>
          ...
          <Accordion>
            <AccordionHeader>
              <Text strong>{t("ui.Personal Key Creation")}</Text>
            </AccordionHeader>
            <AccordionBody>
              <Form.Item
                className="gap-2"
                label={
                  <span>
                    {t("ui.Models")}{" "}
                    <Tooltip title={t("ui.Models user has access to, outside of team scope.")}>
                      <InfoCircleOutlined style={{ marginLeft: "4px" }} />
                    </Tooltip>
                  </span>
                }
                name="models"
                help={t("ui.Models user has access to, outside of team scope.")}
              >
                <Select2 mode="multiple" placeholder={t("ui.Select models")} style={{ width: "100%" }}>
