...
const EmailInvitationsNotice: React.FC = () => {
  ...
  return (
    <Alert variant="info" className="mb-4">
      <Info />
      <AlertTitle>{t("ui.Email invitations")}</AlertTitle>
      <AlertDescription>
        {t(
          "ui.New users receive an email invite only when an email integration (SMTP, Resend, or SendGrid) is configured.",
        )}{" "}
        <a href="https://docs.litellm.ai/docs/proxy/email" target="_blank" rel="noreferrer">
          {t("ui.Learn how to set up email notifications")}
        </a>
      </AlertDescription>
    </Alert>
  );
...
export const CreateUserButton: React.FC<CreateuserProps> = ({
  ...
  const handleCreate = async (formValues: CreateUserFormValues) => {
    try {
      ...
      toast.success(t("ui.API user Created"));
      form.reset(defaultValues);
      localStorage.removeItem("userData" + userID);
    } catch (error: any) {
      const errorMessage = error.response?.data?.detail || error?.message || t("ui.Error creating the user");
  ...
  const userEmailField = (
    <FormField control={form.control} name="user_email" label={t("ui.User Email")}>
  ...
  const teamField = (
    <FormField
      ...
      name="team_id"
      label={t("ui.Team")}
      description={t("ui.If selected, user will be added as a 'user' role to the team.")}
    >
  ...
  const metadataField = (
    <FormField control={form.control} name="metadata" label={t("ui.Metadata")}>
      {({ ref, value, ...control }) => (
        <Textarea {...control} ref={ref} value={value ?? ""} rows={4} placeholder={t("ui.Enter metadata as JSON")} />
      )}
    </FormField>
  );

  const sendInviteEmailField = (
    <FormField control={form.control} name="send_invite_email" label={t("ui.Send invitation email")}>
    ...
    return (
      <TooltipProvider>
        <form onSubmit={form.handleSubmit(handleCreate)}>
          ...
          <FieldGroup>
            {userEmailField}
            {roleField(t("ui.User Role"))}
            {teamField}
            {metadataField}
            {sendInviteEmailField}
          </FieldGroup>
          <div className="mt-4 text-right">
            <Button type="submit">{t("ui.Create User")}</Button>
          </div>
        </form>
      </TooltipProvider>
    );
    ...
    <>
      ...
      <Dialog open={isModalVisible} onOpenChange={(open) => !open && handleCancel()}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>{t("ui.Invite User")}</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-3">
            <p className="mb-1 text-sm text-foreground">{t("ui.Create a User who can own keys")}</p>
            <EmailInvitationsNotice />
          </div>
          <TooltipProvider>
            <form onSubmit={form.handleSubmit(handleCreate)}>
              <FieldGroup>
                ...
                {roleField(
                  labelWithHint(
                    ...
                    t(
                      "ui.This role is independent of any team/org specific roles. Configure Team / Organization Admins in the Settings",
                    ),
                  ),
                )}
                {teamField}
                ...
                <FormField
                  ...
                  description={t("ui.The user will be added to the selected organization(s).")}
                >
                  {({ id, value, onChange }) => (
                    <Select
                      multiple
                    ...
                    >
                      <SelectTrigger id={id} className="w-full">
                        <SelectValue placeholder={t("ui.Select Organization")}>
                          {(selected: string[]) =>
                            selected.length === 0
                              ? t("ui.Select Organization")
                              : organizationOptions
                ...
                <Collapsible open={isPersonalKeyOpen} onOpenChange={setIsPersonalKeyOpen}>
                  <CollapsibleTrigger className="flex w-full items-center gap-2 rounded-md border border-border px-3 py-2 text-left text-sm font-semibold text-foreground">
                    <ChevronRight
                      ...
                      aria-hidden
                    />
                    {t("ui.Personal Key Creation")}
                  </CollapsibleTrigger>
                  <CollapsibleContent className="pt-4">
                    <FormField
                      control={form.control}
                      name="models"
                      label={labelWithHint(t("ui.Models"), t("ui.Models user has access to, outside of team scope."))}
                      description={t("ui.Models user has access to, outside of team scope.")}
                    >
                      {({ value, onChange }) => (
                        <MultiSelect
                          options={[
                            { label: t("ui.All Proxy Models"), value: "all-proxy-models" },
                            { label: t("ui.No Default Models"), value: "no-default-models" },
