...
const OrganizationInfoView: React.FC<OrganizationInfoProps> = ({
  ...
  const handleMemberAdd = async (values: any) => {
    try {
      ...
      NotificationsManager.success(t("ui.Organization member added successfully"));
      setIsAddMemberModalVisible(false);
      queryClient.invalidateQueries({ queryKey: organizationKeys.all });
    } catch (error) {
      NotificationsManager.fromBackend(t("ui.Failed to add organization member"));
  ...
  const handleMemberUpdate = async (values: any) => {
    try {
      ...
      NotificationsManager.success(t("ui.Organization member updated successfully"));
      setIsEditMemberModalVisible(false);
      queryClient.invalidateQueries({ queryKey: organizationKeys.all });
    } catch (error) {
      NotificationsManager.fromBackend(t("ui.Failed to update organization member"));
  ...
  const handleMemberDelete = async (values: any) => {
    try {
      ...
      NotificationsManager.success(t("ui.Organization member deleted successfully"));
      setIsEditMemberModalVisible(false);
      queryClient.invalidateQueries({ queryKey: organizationKeys.all });
    } catch (error) {
      NotificationsManager.fromBackend(t("ui.Failed to delete organization member"));
  ...
  if (!orgData) {
    return <div className="p-4">{t("ui.Organization not found")}</div>;
  }

  const orgExtraColumns: MemberTableColumn[] = [
    {
      title: t("ui.Spend (USD)"),
      key: "spend",
  ...
  return (
    <div className="h-screen w-full bg-background p-4">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-1">
            ...
            <CopyButton value={orgData.organization_id} label={t("ui.Copy organization ID")} iconClassName="size-3" />
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue={editOrg ? "settings" : "overview"} onValueChange={onTabChange} className="mb-4">
        ...
        <TabsContent keepMounted={hasVisited("overview")} value="overview" className="pt-4">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <Card>
              <CardContent>
                <p className="text-sm text-muted-foreground">{t("ui.Organization Details")}</p>
                <div className="mt-2 text-sm text-foreground">
                  <p>{t("ui.Created: {{date}}", { date: new Date(orgData.created_at).toLocaleDateString() })}</p>
                  <p>{t("ui.Updated")}: {new Date(orgData.updated_at).toLocaleDateString()}</p>
                  <p>{t("ui.Created By: {{user}}", { user: orgData.created_by })}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-muted-foreground">{t("ui.Budget Status")}</p>
                <div className="mt-2 text-sm text-foreground">
                  ...
                  {orgData.litellm_budget_table.budget_duration && (
                    <p className="text-muted-foreground">
                      {t("ui.Reset: {{duration}}", { duration: orgData.litellm_budget_table.budget_duration })}
                    </p>
                  )}
                </div>
              </CardContent>
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-muted-foreground">{t("ui.Rate Limits")}</p>
                <div className="mt-2 text-sm text-foreground">
                  <p>{t("ui.TPM: {{value}}", { value: orgData.litellm_budget_table.tpm_limit || t("ui.Unlimited") })}</p>
                  <p>{t("ui.RPM: {{value}}", { value: orgData.litellm_budget_table.rpm_limit || t("ui.Unlimited") })}</p>
                  {orgData.litellm_budget_table.max_parallel_requests && (
                    <p>
                      {t("ui.Max Parallel Requests: {{count}}", {
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-muted-foreground">{t("ui.Models")}</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {orgData.models.length === 0 ? (
                    <BadgeLink>{t("ui.All proxy models")}</BadgeLink>
                  ) : (
            ...
            <Card>
              <CardContent>
                <p className="text-sm text-muted-foreground">{t("ui.Teams")}</p>
                <div className="mt-2 flex flex-wrap gap-2">
        ...
        <TabsContent keepMounted={hasVisited("members")} value="members" className="pt-4">
          <div className="space-y-4">
            <MemberTable
              ...
              roleColumnTitle={t("ui.Organization Role")}
              extraColumns={orgExtraColumns}
              emptyText={t("ui.No members found")}
            />
          </div>
        </TabsContent>
          ...
          <Card className="max-h-[65vh] overflow-y-auto">
            <CardContent>
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-foreground">{t("ui.Organization Settings")}</h2>
              ...
              {isEditing ? (
                ...
                <div className="space-y-4 text-sm">
                  <div>
                    <p className="font-medium text-foreground">{t("ui.Organization Name")}</p>
                    <div>{orgData.organization_alias}</div>
                  </div>
                  <div>
                    <p className="font-medium text-foreground">{t("ui.Organization ID")}</p>
                    <div className="font-mono">{orgData.organization_id}</div>
                  </div>
                  <div>
                    <p className="font-medium text-foreground">{t("ui.Created At")}</p>
                    <div>{new Date(orgData.created_at).toLocaleString()}</div>
                  </div>
                  <div>
                    <p className="font-medium text-foreground">{t("ui.Models")}</p>
                  ...
                  <div>
                    <p className="font-medium text-foreground">{t("ui.Rate Limits")}</p>
                    <div>{t("ui.TPM: {{value}}", { value: orgData.litellm_budget_table.tpm_limit || t("ui.Unlimited") })}</div>
                    <div>{t("ui.RPM: {{value}}", { value: orgData.litellm_budget_table.rpm_limit || t("ui.Unlimited") })}</div>
                  </div>
                  <div>
                    <p className="font-medium text-foreground">{t("ui.Budget")}</p>
                    <div>
                      {t("ui.Max: {{value}}", {
                        value:
                          orgData.litellm_budget_table.max_budget !== null
                    ...
                    <div>
                      {t("ui.Reset: {{value}}", {
                        value: orgData.litellm_budget_table.budget_duration || t("ui.Never"),
                      })}
                    </div>
                  </div>
      ...
      <UserSearchModal
        ...
        accessToken={accessToken}
        title={t("ui.Add Organization Member")}
        roles={[
          {
            label: "org_admin",
            value: "org_admin",
            description: t("ui.Can add and remove members, and change their roles."),
          },
          {
            label: "internal_user",
            value: "internal_user",
            description: t("ui.Can view/create keys for themselves within organization."),
          },
          {
            label: "internal_user_viewer",
            value: "internal_user_viewer",
            description: t("ui.Can only view their keys within organization."),
          },
        ]}
      ...
      <MemberModal
        ...
        config={{
          ...
          showUserId: true,
          roleOptions: [
            { label: t("ui.Org Admin"), value: "org_admin" },
            { label: t("ui.Internal User"), value: "internal_user" },
            { label: t("ui.Internal User Viewer"), value: "internal_user_viewer" },
          ],
        }}
      />
    </div>
