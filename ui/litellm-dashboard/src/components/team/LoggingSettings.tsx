    ...
    return (
      <div className="mt-6 pt-4 border-t border-gray-100">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-sm font-medium text-gray-700">{t("ui.Integration Parameters")}</span>
        </div>
        <div className="grid grid-cols-1 gap-4">
          {Object.entries(dynamicParams).map(([paramName, paramType]) => (
            <div key={paramName} className="space-y-2">
              ...
              {paramType === "number" && (
                <span className="text-xs text-gray-500">{t("ui.Value must be between 0 and 1")}</span>
              )}
              {paramType === "number" ? (
                <NumericalInput
                  step={0.01}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-gray-800">{t("ui.Disabled Callbacks")}</span>
          <Tooltip title={t("ui.Select callbacks to disable for this key. Disabled callbacks will not receive any logging data.")}>
            <InfoCircleOutlined className="text-gray-400 cursor-help" />
          </Tooltip>
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">{t("ui.Disabled Callbacks")}</label>
          <Select
            mode="multiple"
            placeholder={t("ui.Select callbacks to disable")}
          ...
          <div className="text-xs text-gray-500">
            {t("ui.Select callbacks that should be disabled for this key. These callbacks will not receive any logging data.")}
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-gray-800">{t("ui.Logging Integrations")}</span>
          <Tooltip title={t("ui.Configure callback logging integrations for this team.")}>
            <InfoCircleOutlined className="text-gray-400 cursor-help" />
          </Tooltip>
        </div>
        <Button
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">
                    {t("ui.{{name}} Configuration", {
                      name: callbackDisplayName || t("ui.New Integration"),
                    })}
                  </span>
                </div>
                <Button
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">{t("ui.Integration Type")}</label>
                    <Select
                      value={callbackDisplayName}
                      placeholder={t("ui.Select integration")}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">{t("ui.Event Type")}</label>
                    <Select
                    ...
                    >
                      <Option value="success">
                        <div className="flex items-center space-x-2">
                          ...
                          <span>{t("ui.Success Only")}</span>
                        </div>
                      </Option>
                      <Option value="failure">
                        <div className="flex items-center space-x-2">
                          <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                          <span>{t("ui.Failure Only")}</span>
                        </div>
                      </Option>
                      <Option value="success_and_failure">
                        <div className="flex items-center space-x-2">
                          <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                          <span>{t("ui.Success & Failure")}</span>
                        </div>
                      </Option>
                    </Select>
                  </div>
      ...
      {value.length === 0 && (
        <div className="text-center py-12 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50/50">
          ...
          <div className="text-base font-medium mb-1">{t("ui.No logging integrations configured")}</div>
          <div className="text-sm text-gray-400">
            {t('ui.Click "Add Integration" to configure logging for this team')}
          </div>
        </div>
      )}
    </div>
