    ...
    <div className="flex w-full flex-col gap-2 p-3 text-sm">
        ...
        {premiumUser ? (
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="left">{t("ui.Upgrade to Premium for advanced features")}</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      </div>
      ...
      <div className="flex w-full items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground">{t("ui.User ID")}</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="max-w-[150px] truncate" title={userId || "-"}>
            {userId || "-"}
          </span>
          <CopyButton value={userId} label={t("ui.Copy User ID")} />
        </div>
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <ShieldCheck className="size-4" />
          <span className="text-muted-foreground">{t("ui.Role")}</span>
        </div>
        <span>{userRole}</span>
      </div>
      <Separator className="my-2" />
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">{t("ui.Hide New Feature Indicators")}</span>
        <Switch
          size="sm"
          ...
          aria-label={t("ui.Toggle hide new feature indicators")}
        />
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">{t("ui.Hide All Prompts")}</span>
        <Switch
          size="sm"
          ...
          aria-label={t("ui.Toggle hide all prompts")}
        />
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">{t("ui.Hide Blog Posts")}</span>
        <Switch
          size="sm"
          ...
          aria-label={t("ui.Toggle hide blog posts")}
        />
      </div>
      <div className="flex w-full items-center justify-between gap-2">
        <span className="text-muted-foreground">{t("ui.Hide Bouncing Icon")}</span>
        <Switch
          size="sm"
          ...
          aria-label={t("ui.Toggle hide bouncing icon")}
        />
      </div>
    </div>
  );
