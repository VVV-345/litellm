      ...
      updateSettings(formValues, {
        onSuccess: () => {
          ...
          NotificationManager.success(
            t("ui.Settings updated successfully. Changes will be applied across all pods within 10 seconds."),
          );
        },
        onError: (error) => {
  ...
  if (!accessToken) {
    return (
      <div className="p-6 text-center text-gray-500">
        {t("ui.Please log in to configure semantic filter settings.")}
      </div>
    );
  }
  ...
  return (
    <div style={{ width: "100%" }}>
      {isLoading ? (
        ...
        <Alert
          type="error"
          message={t("ui.Could not load MCP Semantic Filter settings")}
        ...
        <>
          <Alert
            type="info"
            message={t("ui.Semantic Tool Filtering")}
            description={t(
              "ui.Filter MCP tools semantically based on query relevance. This reduces context window size and improves tool selection accuracy. Click 'Save Settings' to apply changes across all pods (takes effect within 10 seconds).",
            )}
            showIcon
            style={{ marginBottom: 24 }}
          />
          ...
          {saveSuccess && (
            <Alert
              type="success"
              message={t("ui.Settings saved successfully")}
          ...
          {updateError && (
            <Alert
              type="error"
              message={t("ui.Could not update settings")}
              ...
              >
                <Card style={{ marginBottom: 16 }}>
                  <Form.Item
                    ...
                    label={
                      <Space>
                        <Typography.Text strong>{t("ui.Enable Semantic Filtering")}</Typography.Text>
                        <Tooltip
                          title={t(
                            "ui.When enabled, only the most relevant MCP tools will be included in requests based on semantic similarity",
                          )}
                        >
                ...
                <Card title={t("ui.Configuration")} style={{ marginBottom: 16 }}>
                  <Form.Item
                    name="embedding_model"
                    label={
                      <Space>
                        <Typography.Text strong>{t("ui.Embedding Model")}</Typography.Text>
                        <Tooltip title={t("ui.The model used to generate embeddings for semantic matching")}>
                          <QuestionCircleOutlined style={{ color: "#8c8c8c" }} />
                        </Tooltip>
                      </Space>
                    }
                  >
                    <Select
                      ...
                      placeholder={loadingModels ? t("ui.Loading models...") : t("ui.Select embedding model")}
                      showSearch
                      disabled={isUpdating || loadingModels}
                      loading={loadingModels}
                      notFoundContent={loadingModels ? t("ui.Loading...") : t("ui.No embedding models available")}
                    />
                  </Form.Item>
                  ...
                  <Form.Item
                    ...
                    label={
                      <Space>
                        <Typography.Text strong>{t("ui.Top K Results")}</Typography.Text>
                        <Tooltip title={t("ui.Maximum number of tools to return after filtering")}>
                          <QuestionCircleOutlined style={{ color: "#8c8c8c" }} />
                        </Tooltip>
                      </Space>
                    }
                  ...
                  <Form.Item
                    ...
                    label={
                      <Space>
                        <Typography.Text strong>{t("ui.Similarity Threshold")}</Typography.Text>
                        <Tooltip title={t("ui.Minimum similarity score (0-1) for a tool to be included")}>
                          <QuestionCircleOutlined style={{ color: "#8c8c8c" }} />
                        </Tooltip>
                      </Space>
                    }
