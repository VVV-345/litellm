...
export default function UISettings() {
  ...
  const handleToggle = (checked: boolean) => {
    updateSettings(
      ...
      {
        onSuccess: () => {
          NotificationManager.success(t("ui.UI settings updated successfully"));
        },
        onError: (error) => {
  ...
  const handleUpdatePageVisibility = (settings: { enabled_ui_pages_internal_users: string[] | null }) => {
    updateSettings(settings, {
      onSuccess: () => {
        NotificationManager.success(t("ui.Page visibility settings updated successfully"));
      },
      onError: (error) => {
        NotificationManager.fromBackend(error);
      },
  ...
  const handleToggleEnableProjectsUI = (checked: boolean) => {
    updateSettings(
      ...
      {
        onSuccess: () => {
          NotificationManager.success(t("ui.UI settings updated successfully. Refreshing page..."));
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          <h3>{t("ui.UI Settings")}</h3>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div role="status" aria-label={t("ui.Loading UI settings")} className="space-y-3">
          ...
          <Alert variant="error">
            <AlertTitle>{t("ui.Could not load UI settings")}</AlertTitle>
          ...
          <div className="space-y-6">
            ...
            {updateError && (
              <Alert variant="error">
                <AlertTitle>{t("ui.Could not update UI settings")}</AlertTitle>
            ...
            <SettingRow
              ...
              ariaLabel={property?.description ?? t("ui.Disable model add for internal users")}
              label={t("ui.Disable model add for internal users")}
            ...
            <SettingRow
              ...
              ariaLabel={disableTeamAdminDeleteProperty?.description ?? t("ui.Disable team admin delete team user")}
              label={t("ui.Disable team admin delete team user")}
            ...
            <SettingRow
              ...
              ariaLabel={
                requireAuthForPublicAIHubProperty?.description ?? t("ui.Require authentication for public AI Hub")
              }
              label={t("ui.Require authentication for public AI Hub")}
            ...
            <SettingRow
              ...
              ariaLabel={forwardClientHeadersProperty?.description ?? t("ui.Forward client headers to LLM API")}
              label={t("ui.Forward client headers to LLM API")}
              description={
                forwardClientHeadersProperty?.description ??
                t(
                  "ui.Forwards client headers (Authorization, anthropic-beta, and x-* custom headers) to the upstream LLM. Enable for Claude Code with a Max subscription (forwards the OAuth token) or to pass custom/tracing headers through to the provider. Independent of the BYOK toggle — enable only the one(s) you need.",
                )
              }
            />
            <SettingRow
            ...
            {enableProjectsUIProperty && (
              <SettingRow
                ...
                ariaLabel={enableProjectsUIProperty.description ?? t("ui.Enable Projects UI")}
                label={t("ui.[BETA] Enable Projects (page will refresh)")}
                description={
                  enableProjectsUIProperty.description ??
                  t(
                    "ui.If enabled, shows the Projects feature in the UI sidebar and the project field in key management.",
                  )
                }
              />
            )}
            <SettingRow
              ...
              ariaLabel={enableChatUIProperty?.description ?? t("ui.Enable Chat page")}
              label={t("ui.[BETA] Enable Chat page (page will refresh)")}
              description={
                enableChatUIProperty?.description ??
                t(
                  "ui.If enabled, shows the Chat page in the UI sidebar, letting users chat with an LLM and connect their own MCP server credentials via OAuth.",
                )
              }
            />
            ...
            <SettingRow
              ...
              ariaLabel={disableAgentsProperty?.description ?? t("ui.Disable agents for internal users")}
              label={t("ui.Disable agents for internal users")}
            ...
            <SettingRow
              ...
              ariaLabel={allowAgentsTeamAdminsProperty?.description ?? t("ui.Allow agents for team admins")}
              label={t("ui.Allow agents for team admins")}
            ...
            <SettingRow
              ...
              ariaLabel={disableVectorStoresProperty?.description ?? t("ui.Disable vector stores for internal users")}
              label={t("ui.Disable vector stores for internal users")}
            ...
            <SettingRow
              ...
              ariaLabel={
                allowVectorStoresTeamAdminsProperty?.description ?? t("ui.Allow vector stores for team admins")
              }
              label={t("ui.Allow vector stores for team admins")}
            ...
            <SettingRow
              ...
              ariaLabel={scopeUserSearchProperty?.description ?? t("ui.Scope user search to organization")}
              label={t("ui.Scope user search to organization")}
              description={
                scopeUserSearchProperty?.description ??
                t(
                  "ui.If enabled, the user search endpoint restricts results by organization. When off, any authenticated user can search all users.",
                )
              }
            />
            ...
            <SettingRow
              ...
              ariaLabel={disableCustomApiKeysProperty?.description ?? t("ui.Disable custom Virtual key values")}
              label={t("ui.Disable custom Virtual key values")}
              description={
                disableCustomApiKeysProperty?.description ??
                t("ui.If true, users cannot specify custom key values. All keys must be auto-generated.")
              }
            />
