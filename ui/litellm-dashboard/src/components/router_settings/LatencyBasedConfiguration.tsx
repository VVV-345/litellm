...
const LatencyBasedConfiguration: React.FC<LatencyBasedConfigurationProps> = ({ routingStrategyArgs }) => {
  ...
  const paramExplanation: { [key: string]: string } = {
    ttl: t("ui.Sliding window to look back over when calculating the average latency of a deployment. Default - 1 hour (in seconds)."),
    lowest_latency_buffer: t(
      "ui.Shuffle between deployments within this % of the lowest latency. Default - 0 (i.e. always pick lowest latency).",
    ),
  };
    ...
    <>
      <div className="space-y-6">
        <div className="max-w-3xl">
          <h3 className="text-sm font-medium text-gray-900">{t("ui.Latency-Based Configuration")}</h3>
          <p className="text-xs text-gray-500 mt-1">{t("ui.Fine-tune latency-based routing behavior")}</p>
        </div>
