...
export function LoggingSettingsView({
  ...
  const getEventTypeLabel = (eventType: string) => {
    switch (eventType) {
      case "success":
        return t("ui.Success Only");
      case "failure":
        return t("ui.Failure Only");
      case "success_and_failure":
        return t("ui.Success & Failure");
      default:
        return eventType;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900">{t("ui.Logging Integrations")}</span>
        ...
        {loggingConfigs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-blue-600">
                        {t("ui.{{count}} parameters configured", { count: Object.keys(config.callback_vars).length })}
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <span className="text-gray-500 text-sm">{t("ui.No logging integrations configured")}</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-900">{t("ui.Disabled Callbacks")}</span>
        ...
        {disabledCallbacks.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-red-600">{t("ui.Disabled for this key")}</span>
                    </div>
                  </div>
                  <Badge variant="destructive">{t("ui.Disabled")}</Badge>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <span className="text-gray-500 text-sm">{t("ui.No callbacks disabled")}</span>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className={`bg-white border border-gray-200 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <span className="block font-semibold text-gray-900">{t("ui.Logging Settings")}</span>
            <span className="block text-xs text-gray-500">
              {t("ui.Active logging integrations and disabled callbacks for this key")}
            </span>
          </div>
        </div>
        {content}
  ...
  return (
    <div className={`${className}`}>
      <span className="block font-medium text-gray-900 mb-3">{t("ui.Logging Settings")}</span>
      {content}
    </div>
  );
}
