...
function AgentHubRowActions({ agent, onAgentClick }: AgentHubRowActionsProps) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        aria-label={ui("Open agent actions")}
      ...
      <DropdownMenuContent align="end" className="w-52">
        ...
        <DropdownMenuItem
          ...
          onClick={() => void copyToClipboard(agent.name, ui("Agent name copied"))}
        >
          <Copy />
          {ui("Copy agent name")}
        </DropdownMenuItem>
...
export const getAgentHubTableColumns = ({ onAgentClick, t }: AgentHubTableColumnsDeps): ColumnDef<AgentHubData>[] => {
  const ui = (value: string, options?: Record<string, unknown>) =>
    t ? t(`ui.${value}`, { defaultValue: value, ...options }) : value;
  return [
    {
      id: "name",
      accessorKey: "name",
      meta: { title: ui("Agent Name") },
      header: ({ column }) => <DataTableSortHeader column={column} title={ui("Agent Name")} />,
      size: 200,
      enableSorting: true,
    ...
    },
    {
      id: "version",
      accessorKey: "version",
      meta: { title: ui("Version"), skeleton: "badge", className: "hidden lg:table-cell" },
      header: ({ column }) => <DataTableSortHeader column={column} title={ui("Version")} />,
      size: 100,
      enableSorting: true,
      sortingFn: "alphanumeric",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      meta: { title: ui("Protocol"), className: "hidden lg:table-cell" },
      header: ({ column }) => <DataTableSortHeader column={column} title={ui("Protocol")} />,
      size: 100,
      enableSorting: true,
      sortingFn: "alphanumeric",
    ...
    {
      id: "skills",
      meta: { title: ui("Skills"), skeleton: "chips" },
      header: ui("Skills"),
      size: 180,
      enableSorting: false,
      cell: ({ row }) => {
        const skills = row.original.skills || [];
        const skillCountKey = skills.length === 1 ? "{{count}} skill" : "{{count}} skills";
        return (
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium">{ui(skillCountKey, { count: skills.length })}</span>
            {skills.length > 0 && (
    ...
    {
      id: "capabilities",
      meta: { title: ui("Capabilities"), skeleton: "chips" },
      header: ui("Capabilities"),
      size: 160,
      enableSorting: false,
      cell: ({ row }) => {
    ...
    {
      id: "io_modes",
      meta: { title: ui("I/O Modes"), skeleton: "twoLine", className: "hidden xl:table-cell" },
      header: ui("I/O Modes"),
      size: 150,
      enableSorting: false,
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-0.5 text-xs">
            <span>
              <span className="font-medium">{ui("In:")}</span> {inputModes.join(", ") || "-"}
            </span>
            <span>
              <span className="font-medium">{ui("Out:")}</span> {outputModes.join(", ") || "-"}
            </span>
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <StatusBadge tone={isPublic ? "success" : "neutral"} label={isPublic ? ui("Yes") : ui("No")} />;
      },
    },
    {
      id: "actions",
