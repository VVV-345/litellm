      ...
      <CardContent className="flex h-full flex-col">
        ...
        {inherits && (
          <div className="mb-4 text-xs">
            <span className="text-muted-foreground">{t("ui.Inherits from:")} </span>
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        toast.error(t("ui.Failed to fetch policy templates"));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-lg font-medium">{t("ui.Policy Templates")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("ui.Start with a pre-configured policy template to quickly set up guardrails for your organization.")}
          </p>
        </div>
          ...
          <div className="w-52 shrink-0">
            <div className="sticky top-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold">{t("ui.Categories")}</span>
                {selectedTags.size > 0 && (
        ...
        <div className="flex-1">
          {selectedTags.size > 0 && (
            <div className="mb-4 text-sm text-muted-foreground">
              {t("ui.Showing")} {filteredTemplates.length} {t("ui.of")} {templates.length} templates
            </div>
          )}
          ...
          {filteredTemplates.length === 0 && (
            <div className="py-12 text-center text-muted-foreground">
              <p>{t("ui.No templates match the selected filters.")}</p>
