...
const TagEditForm: React.FC<TagEditFormProps> = ({ tag, seedBudgetFields, userModels, onCancel, onSave }) => {
  ...
  return (
    <form onSubmit={form.handleSubmit(submitVisibleValues)} noValidate>
      <FieldGroup>
        <FormField control={form.control} name="name" label={t("ui.Tag Name")}>
        ...
        <FormField
          ...
          name="models"
          label={t("ui.Allowed Models")}
          description={t("ui.Select which models are allowed to process this type of data")}
        >
          {({ value, onChange }) => (
            <MultiSelect
        ...
        <CollapsibleContent className="px-4 pb-4">
          <FieldGroup className="mt-4">
            <FormField
              ...
              description={t("ui.Maximum amount in USD this tag can spend")}
            >
            ...
            <FormField
              ...
              description={t("ui.How often the budget should reset")}
            >
              {({ id, value, onChange }) => (
...
const TagInfoView: React.FC<TagInfoViewProps> = ({ tagId, onClose, accessToken, is_admin, editTag }) => {
  ...
  const handleSave = async (values: TagEditFormValues) => {
    ...
    try {
      ...
      toast.success(t("ui.Tag updated successfully"));
      setIsEditing(false);
      fetchTagDetails();
    } catch (error) {
    ...
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">{t("ui.Tag Name:")}</span>
      ...
      {isEditing ? (
        ...
        <div className="space-y-6">
          <Card>
            <CardContent>
              <CardTitle>{t("ui.Tag Details")}</CardTitle>
              <div className="space-y-4 mt-4">
                ...
                <div>
                  <p className="font-medium">{t("ui.Allowed Models")}</p>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {!tagDetails.models || tagDetails.models.length === 0 ? (
                      <Badge variant="secondary">{t("ui.All Models")}</Badge>
                    ) : (
          ...
          {tagDetails.litellm_budget_table && (
            <Card>
              <CardContent>
                <CardTitle>{t("ui.Budget & Rate Limits")}</CardTitle>
                <div className="space-y-4 mt-4">
                  ...
                  {tagDetails.litellm_budget_table.budget_duration && (
                    <div>
                      <p className="font-medium">{t("ui.Budget Duration")}</p>
                  ...
                  {tagDetails.litellm_budget_table.tpm_limit !== undefined &&
                    tagDetails.litellm_budget_table.tpm_limit !== null && (
                      <div>
                        <p className="font-medium">{t("ui.TPM Limit")}</p>
                  ...
                  {tagDetails.litellm_budget_table.rpm_limit !== undefined &&
                    tagDetails.litellm_budget_table.rpm_limit !== null && (
                      <div>
                        <p className="font-medium">{t("ui.RPM Limit")}</p>
