          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Label htmlFor={serverHeaderToggleId} className="font-normal leading-normal">
                {t("ui.Limit tools to specific MCP servers or MCP groups by passing the")} <code>x-mcp-servers</code>{" "}
                {t("ui.header")}
              </Label>
            </div>
            {useServerHeader && (
              <Alert className="mt-2" variant="info">
                <Info />
                <AlertTitle>{t("ui.Two Options")}</AlertTitle>
                <AlertDescription>
                  <div>
                    <p>
                      <strong>{t("ui.Option 1:")}</strong> {t("ui.Get a specific server:")}{" "}
                      <code>"{serverName.replace(/\s+/g, "_")}"</code>
                    </p>
                    <p>
                      <strong>{t("ui.Option 2:")}</strong> {t("ui.Get a group of MCPs:")} <code>"dev-group"</code>
                    </p>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("ui.You can also mix both:")} <code>"Server1,dev-group"</code>
                    </p>
                  </div>
                </AlertDescription>
              </Alert>
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-success/15 to-success/5 p-6 rounded-lg border border-success/15">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="mb-0 text-xl font-semibold text-success">{t("ui.LiteLLM Proxy API Integration")}</h4>
        </div>
        <span className="text-success">
          {t("ui.Connect to LiteLLM Proxy Responses API for seamless tool integration with multiple model providers")}
        </span>
      </div>
      ...
      <div className="flex w-full flex-col gap-6">
        <FeatureCard
          ...
          title={t("ui.Virtual Key Setup")}
          description={t("ui.Configure your LiteLLM Proxy Virtual Key for authentication")}
        >
          <div className="flex w-full flex-col gap-4">
            <div>
              <span>
                {t("ui.Get your Virtual Key from your LiteLLM Proxy dashboard or contact your administrator")}
              </span>
            </div>
            <CodeBlock
              title={t("ui.Environment Variable")}
        ...
        <FeatureCard
          ...
          title={t("ui.MCP Server Information")}
          description={t("ui.Connection details for your LiteLLM MCP server")}
        >
          <CodeBlock title={t("ui.Server URL")} code={`${proxyBaseUrl}/mcp`} copyKey="litellm-server-url" />
        </FeatureCard>

        <FeatureCard
          icon={<Code className="text-success" size={16} />}
          title={t("ui.Implementation Example")}
          description={t("ui.Complete cURL example for using the LiteLLM Proxy Responses API")}
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-info/15 to-info/5 p-6 rounded-lg border border-info/15">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="mb-0 text-xl font-semibold text-info">{t("ui.OpenAI Responses API Integration")}</h4>
        </div>
        <span className="text-info">
          {t("ui.Connect OpenAI Responses API to your LiteLLM MCP server for seamless tool integration")}
        </span>
      </div>
      ...
      <div className="flex w-full flex-col gap-6">
        <FeatureCard
          ...
          title={t("ui.API Key Setup")}
          description={t("ui.Configure your OpenAI API key for authentication")}
        >
          <div className="flex w-full flex-col gap-4">
            <div>
              <span>
                {t("ui.Get your API key from the")}{" "}
                <a
                ...
                >
                  {t("ui.OpenAI platform")} <ExternalLinkIcon size={12} />
                </a>
              </span>
            </div>
            <CodeBlock
        ...
        <FeatureCard
          ...
          description={t("ui.Connection details for your LiteLLM MCP server")}
        >
          <CodeBlock title={t("ui.Server URL")} code={`${proxyBaseUrl}/mcp`} copyKey="openai-server-url" />
        </FeatureCard>

        <FeatureCard
          icon={<Code className="text-info" size={16} />}
          title={t("ui.Implementation Example")}
          description={t("ui.Complete cURL example for using the Responses API")}
...
--data '{
  ...
  const CursorTab = () => (
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-purple-50 to-blue-50 p-6 rounded-lg border border-purple-100 dark:from-purple-950 dark:to-blue-950 dark:border-purple-900">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="mb-0 text-xl font-semibold text-purple-900 dark:text-purple-100">
            {t("ui.Cursor IDE Integration")}
          </h4>
        </div>
        <span className="text-purple-700 dark:text-purple-300">
          {t(
            "ui.Use tools directly from Cursor IDE with LiteLLM MCP. Enable your AI assistant to perform real-world tasks without leaving your coding environment.",
          )}
        </span>
      </div>
      ...
      <Card>
        <CardContent>
          <h5 className="mb-4 text-base font-semibold text-foreground">{t("ui.Setup Instructions")}</h5>
          <div className="flex w-full flex-col gap-6">
            <StepCard step={1} title={t("ui.Open Cursor Settings")}>
              <span className="text-muted-foreground">
                {t("ui.Use the keyboard shortcut")} <code className="bg-muted px-2 py-1 rounded-sm">⇧+⌘+J</code>{" "}
                {t("ui.(Mac) or")} <code className="bg-muted px-2 py-1 rounded-sm">Ctrl+Shift+J</code>{" "}
                {t("ui.(Windows/Linux)")}
              </span>
            </StepCard>

            <StepCard step={2} title={t("ui.Navigate to MCP Tools")}>
              <span className="text-muted-foreground">{t("ui.Go to the \"MCP Tools\" tab and click \"New MCP Server\"")}</span>
            </StepCard>

            <StepCard step={3} title={t("ui.Add Configuration")}>
              <span className="mb-3 text-muted-foreground">
                {t("ui.Copy the JSON configuration below and paste it into Cursor, then save with")}{" "}
                <code className="bg-muted px-2 py-1 rounded-sm">Cmd+S</code> {t("ui.or")}{" "}
                <code className="bg-muted px-2 py-1 rounded-sm">Ctrl+S</code>
              </span>
              <FeatureCard
                icon={<Code className="text-purple-600 dark:text-purple-400" size={16} />}
                title={t("ui.Configuration")}
                description={t("ui.Cursor MCP configuration")}
                serverName="Zapier Gmail"
                accessGroups={["dev-group"]}
                showServerHeaderToggle
              >
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-success/15 to-success/5 p-6 rounded-lg border border-success/15">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="mb-0 text-xl font-semibold text-success">{t("ui.Streamable HTTP Transport")}</h4>
        </div>
        <span className="text-success">
          {t(
            "ui.Connect to LiteLLM MCP using HTTP transport. Compatible with any MCP client that supports HTTP streaming.",
          )}
        </span>
      </div>
      ...
      <FeatureCard
        ...
        title={t("ui.Universal MCP Connection")}
        description={t("ui.Use this URL with any MCP client that supports HTTP transport")}
      >
        <div className="flex w-full flex-col gap-4">
          <div>
            <span>
              {t(
                "ui.Each MCP client supports different transports. Refer to your client documentation to determine the appropriate transport method.",
              )}
            </span>
          </div>
          <CodeBlock title={t("ui.Server URL")} code={`${proxyBaseUrl}/mcp`} copyKey="http-server-url" />
          <CodeBlock
            title={t("ui.Headers Configuration")}
            code={JSON.stringify(
              {
                "x-litellm-api-key": "Bearer YOUR_LITELLM_API_KEY",
              },
          ...
          <div className="mt-4">
            ...
            >
              ...
              {t("ui.Learn more about MCP transports")}
            </Button>
          </div>
        </div>
      </FeatureCard>
  ...
  return (
    <div>
      <div className="flex w-full flex-col gap-6">
        <div>
          <h2 className="text-3xl font-bold text-foreground mb-3">{t("ui.Connect to your MCP client")}</h2>
          <p className="text-lg text-muted-foreground">
            {t(
              "ui.Use tools directly from any MCP client with LiteLLM MCP. Enable your AI assistant to perform real-world tasks through a simple, secure connection.",
            )}
          </p>
        </div>
