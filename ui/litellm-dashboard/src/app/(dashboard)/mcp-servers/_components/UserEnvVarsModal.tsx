...
const buildSchema = (required: readonly MCPUserEnvVarSpec[], requiredMessage: (name: string) => string) =>
  z.object(
    Object.fromEntries(
      required.map((spec) => [spec.name, spec.is_set ? z.string() : z.string().min(1, requiredMessage(spec.name))]),
    ),
  );
...
const UserEnvVarsForm: React.FC<UserEnvVarsFormProps> = ({ required, isSaving, onCancel, onSubmit }) => {
  ...
  const form = useZodForm(buildSchema(required, (name) => t("ui.{{name}} is required", { name })), {
    defaultValues: emptyValues(required),
  });
          ...
          <FormField
            ...
            label={
              <span className="flex items-center gap-2">
                ...
                {spec.is_set && <Badge variant="secondary">{t("ui.Set")}</Badge>}
              </span>
            }
          >
            {(field) => (
              <PasswordInput
                ...
                placeholder={
                  spec.is_set
                    ? t("ui.Enter a new value to overwrite")
                    : spec.description || t("ui.Enter your {{name}}", { name: spec.name })
                }
              />
            )}
          </FormField>
...
const UserEnvVarsModal: React.FC<UserEnvVarsModalProps> = ({ server, open, accessToken, onClose, onSaved }) => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      toast.success(t("ui.Credentials saved"));
      onSaved?.(saved);
      onClose();
    },
    onError: (err) => {
      toast.fromError(
        t("ui.Failed to save env vars: {{error}}", { error: err instanceof Error ? err.message : String(err) }),
      );
    },
  });
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[520px]">
        <DialogHeader>
          <div className="flex items-center gap-2">
            <DialogTitle className="text-base font-semibold">{t("ui.Set your credentials")}</DialogTitle>
            <StatusBadge tone="info" label={t("ui.Per-user")} />
          </div>
        ...
        <div className="mt-2 space-y-4">
          {isLoading ? (
            ...
            <Alert variant="error">
              <CircleAlert />
              <AlertTitle>{t("ui.Failed to load env vars")}</AlertTitle>
            </Alert>
          ) : required.length === 0 ? (
            <Alert variant="info">
              <Info />
              <AlertTitle>{t("ui.No per-user fields configured for this server.")}</AlertTitle>
            </Alert>
          ) : (
            <>
              <span className="block text-sm text-muted-foreground">
                {t(
                  "ui.These values are private to you. Your admin configured this MCP server to require these per-user credentials. Saved values are never shown back; leave an already-set field blank to keep it, or enter a value to set or change it.",
                )}
              </span>
