  ...
  const {
    ...
    onTokenReceived: (token, registeredClient) => {
      ...
      if (isClientForwardedTokenMode(allFieldsValue(form).auth_type)) {
        ...
        toast.success(
          t(
            "ui.Token held for this browser session. Tools can now be previewed and configured; the token is not saved to LiteLLM.",
          ),
        );
        return;
      }
  ...
  const handleCreate = async (values: Record<string, unknown>) => {
    ...
    try {
      if (accessToken != null) {
        ...
        if (isAdmin) {
          toast.success(t("ui.MCP Server created successfully"));
        } else {
          toast.success(t("ui.MCP Server submitted for admin review"), {
      ...
      toast.fromError(
        isAdmin
          ? t("ui.Error creating MCP Server: {{reason}}", { reason })
          : t("ui.Error submitting MCP Server: {{reason}}", { reason }),
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
        <DialogHeader>
          <div className="flex items-center gap-3 border-b border-border pb-4">
            ...
            <img src={mcpLogoImg} alt={t("ui.MCP Logo")} className="size-5 object-contain" />
            <DialogTitle className="text-xl font-semibold">
              {isAdmin ? t("ui.Add New MCP Server") : t("ui.Submit MCP Server for Review")}
            </DialogTitle>
          </div>
        </DialogHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {!isAdmin && (
                  <div className="rounded-md bg-info/10 border border-info/20 px-4 py-3 text-sm text-info">
                    {t(
                      "ui.Your submission will be sent for admin review. Once approved, the server will appear in your MCP Servers list. The request must be made with a team-scoped API key.",
                    )}
                  </div>
                )}
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        ...
                        <SimpleTooltip
                          content={t(
                            "ui.A short, unique identifier for this server. Defaults to the server name if not provided. Cannot contain spaces or hyphens; use underscores instead.",
                          )}
                        >
                          <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                        </SimpleTooltip>
                  ...
                  <MountedFormField
                    label={<span className="text-sm font-medium text-foreground">{t("ui.Description")}</span>}
                    name="description"
                  >
                    {(control) => (
                      <Input
                  ...
                  <MountedFormField
                    label={<span className="text-sm font-medium text-foreground">{t("ui.GitHub / Source URL")}</span>}
                    name="source_url"
                  >
                    {(control) => (
                      <Input
                        {...textControl(control)}
                        placeholder={t("ui.https://github.com/org/mcp-server")}
                        className="rounded-lg border-border focus:border-info focus:ring-ring"
                      />
                    )}
                  </MountedFormField>
                  ...
                  <MountedFormField
                    label={<span className="text-sm font-medium text-foreground">{t("ui.Transport Type")}</span>}
                    name="transport"
                    required
                    rules={{ validate: { required: requiredRule(t("ui.Please select a transport type")) } }}
                  >
                    {(control) => (
                      <Select
                      ...
                      >
                        <SelectTrigger {...selectTriggerControl(control)} className="w-full rounded-lg">
                          <SelectValue placeholder={t("ui.Select transport")} />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {(transportType === "http" || transportType === "sse") && (
                    <MountedFormField
                      label={<span className="text-sm font-medium text-foreground">{t("ui.MCP Server URL")}</span>}
                      name="url"
                      required
                      rules={{
                        validate: {
                  ...
                  {transportType !== "stdio" && transportType !== "" && (
                    <Collapsible defaultOpen className="mb-4">
                      <CollapsibleTrigger className="group flex w-full items-center justify-between gap-4 py-2 text-left">
                        <span className="text-sm font-semibold text-foreground">{t("ui.Authentication settings")}</span>
                      ...
                      <CollapsibleContent keepMounted className="space-y-6 pt-2">
                        <MountedFormField
                          ...
                          name="auth_type"
                          required
                          rules={{ validate: { required: requiredRule(t("ui.Please select an auth type")) } }}
                        >
                          {(control) => (
                            <Select {...selectControl<string>(control)} items={authTypeItems}>
                              <SelectTrigger {...selectTriggerControl(control)} className="w-full rounded-lg">
                                <SelectValue placeholder={t("ui.Select auth type")} />
                              </SelectTrigger>
                              <SelectContent>
                        ...
                        {shouldShowAuthValueField && (
                          <MountedFormField
                            ...
                            rules={{
                              validate: {
                                notWhitespace: notOnlyWhitespace(t("ui.Authentication value cannot be empty whitespace")),
                              },
                            }}
                          >
                            {(control) => (
