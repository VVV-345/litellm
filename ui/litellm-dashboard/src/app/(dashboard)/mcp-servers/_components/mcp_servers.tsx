...
const MCPServers: React.FC<MCPServerProps> = ({ accessToken, userRole, userID }) => {
  ...
  const sortOptions: { value: SortKey; label: string }[] = [
    { value: "created_desc", label: t("ui.Recently created") },
    { value: "updated_desc", label: t("ui.Recently updated") },
    { value: "name_asc", label: t("ui.Name (A-Z)") },
    { value: "health", label: t("ui.Health (unhealthy first)") },
  ];
  ...
  const teamSelectItems = React.useMemo(
    () => ({
      all: isInternalUser ? t("ui.All Available Servers") : t("ui.All Servers"),
      personal: t("ui.Personal"),
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("ui.Deleted MCP Server successfully"));
    ...
    <TooltipProvider>
      <div className="h-full w-full p-6">
        <AlertDialog open={isDeleteModalOpen} onOpenChange={(open) => !open && cancelDelete()}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{t("ui.Delete MCP Server?")}</AlertDialogTitle>
            </AlertDialogHeader>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                {t("ui.This action is permanent and cannot be undone. All associated configurations will be removed.")}
              </p>
                ...
                <dl className="mt-3 space-y-1 rounded-lg border border-border bg-muted p-4">
                  {serverToDelete.server_name && (
                    <div className="flex gap-2">
                      <dt className="text-sm text-muted-foreground">{t("ui.Name")}</dt>
                      <dd className="text-sm font-semibold">{serverToDelete.server_name}</dd>
                    </div>
                  )}
                  <div className="flex gap-2">
                    <dt className="text-sm text-muted-foreground">{t("ui.ID")}</dt>
                    <dd className="font-mono text-xs">{serverToDelete.server_id}</dd>
                  </div>
                  {serverToDelete.url && (
                    <div className="flex gap-2">
                      <dt className="text-sm text-muted-foreground">{t("ui.URL")}</dt>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel disabled={isDeletingServer}>{t("ui.Cancel")}</AlertDialogCancel>
              <Button variant="destructive" disabled={isDeletingServer} onClick={confirmDelete}>
                {isDeletingServer ? t("ui.Deleting...") : t("ui.Delete")}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
              ...
              <div className="w-full h-full">
                <div className="w-full">
                  <div className="flex flex-col space-y-4">
                    <div className="flex items-center gap-6 rounded-lg border border-border bg-card px-4 py-3">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium whitespace-nowrap text-muted-foreground">{t("ui.Team")}</p>
                        <Select
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">
                              {isInternalUser ? t("ui.All Available Servers") : t("ui.All Servers")}
                            </SelectItem>
                            <SelectItem value="personal">{t("ui.Personal")}</SelectItem>
                      ...
                      <div className="flex items-center gap-2">
                            ...
                            <TooltipTrigger
                              render={
                                <CircleHelp
                                  ...
                                  aria-label={t("ui.About access groups")}
                                />
                              }
                            />
                            <TooltipContent>
                              {t(
                                "ui.An MCP Access Group is a set of users or teams that have permission to access specific MCP servers. Use access groups to control and organize who can connect to which servers.",
                              )}
                            </TooltipContent>
                          </Tooltip>
                        </p>
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="all">{t("ui.All Access Groups")}</SelectItem>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <InputGroup className="max-w-80">
                    ...
                    <InputGroupInput
                      placeholder={t("ui.Search by name, alias, URL, or ID")}
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                    />
                  </InputGroup>
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium whitespace-nowrap text-muted-foreground">{t("ui.Sort")}</p>
                    <Select
                      items={sortOptions}
                      value={sortKey}
                  ...
                  <div className="ml-auto text-xs text-muted-foreground">
                    {displayedServers.length} {t("ui.of")} {filteredServers.length} {t("ui.servers")}
                  </div>
                </div>
                <div className="mt-4 w-full">
                  {isLoadingServers ? (
                    <div className="flex items-center justify-center gap-3 rounded-lg border border-dashed border-border bg-card p-12">
                      ...
                      <p className="text-sm text-muted-foreground">{t("ui.Loading MCP servers...")}</p>
                    </div>
                  ) : displayedServers.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-border bg-card p-12 text-center">
                      <p className="text-sm text-muted-foreground">
                        {filteredServers.length === 0
                          ? t("ui.No MCP servers configured. Click '+ Add New MCP Server' to get started.")
                          : t("ui.No servers match the current filters or search.")}
                      </p>
                    </div>
                  ) : (
                    <div
