    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <p className="text-sm text-gray-500 mb-4">
          {t("ui.Are you sure you want to")} {action} <span className="font-medium text-gray-700">&quot;{serverName}&quot;</span>?{" "}
          {isApprove
            ? t("ui.This will activate the server. The submitting user will see it in their MCP Servers list once approved.")
            : rejectBody}
        </p>
        {!isApprove && (
          <textarea
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 pt-4 pb-4">
          <p className="text-xs text-gray-500 mb-4">
            {t("ui.Select which fields must be filled in before a submission is considered compliant. LiteLLM will show ✓ / ✗ for each rule on every submission card below.")}
          </p>
                    ...
                    return (
                      <label key={field.key} className="flex items-start gap-2.5 cursor-pointer group">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-400">{t(`ui.${field.description}`)}</div>
                        </div>
                      </label>
                    );
                  })}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              >
                ...
                {translateUiText(t, statusCfg.label)}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
              <span>
                {t("ui.Transport:")} <span className="text-gray-600">{server.transport ?? "sse"}</span>
              </span>
              <span>·</span>
              <span>
                {t("ui.Submitted by:")} <span className="text-gray-600">{server.submitted_by ?? "—"}</span>
              </span>
              <span>·</span>
              <span>{formatDate(server.submitted_at)}</span>
            </div>
            {approvalStatus === "rejected" && server.review_notes && (
              <p className="text-xs text-red-600 mt-1.5">{t("ui.Rejection reason:")} {server.review_notes}</p>
            )}
          </div>
...
export function MCPSubmissionsTab({ accessToken }: MCPSubmissionsTabProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : translateUiText(t, "Failed to load submissions"));
    } finally {
      setIsLoading(false);
    }
  }, [accessToken]);
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      NotificationsManager.success(translateUiText(t, "Submission rules saved"));
    } catch {
      NotificationsManager.fromBackend(translateUiText(t, "Failed to save submission rules"));
    } finally {
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label={translateUiText(t, "Pending Review")} value={summary.pending_review} color="text-yellow-600" />
        <StatCard label={translateUiText(t, "Active")} value={summary.active} color="text-green-600" />
        <StatCard label={translateUiText(t, "Rejected")} value={summary.rejected} color="text-red-600" />
      </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            type="text"
            placeholder={translateUiText(t, "Search MCP servers...")}
            value={search}
        ...
        >
          ...
          <option value="pending_review">{translateUiText(t, "Pending Review")}</option>
          <option value="active">{translateUiText(t, "Active")}</option>
          <option value="rejected">{translateUiText(t, "Rejected")}</option>
        </select>
      </div>

      <div className="space-y-3">
        {isLoading && (
          <div className="text-center py-12 text-gray-500 text-sm">{translateUiText(t, "Loading submissions…")}</div>
        )}
        {error && <div className="text-center py-12 text-red-600 text-sm">{error}</div>}
        {!isLoading && !error && filtered.length === 0 && (
          <div className="text-center py-12 text-gray-400 text-sm">
            {translateUiText(t, "No MCP server submissions match your filters.")}
          </div>
        )}
        {!isLoading &&
          !error &&
