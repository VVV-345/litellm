...
const TokenEndpointAuthMethodField: React.FC<TokenEndpointAuthMethodFieldProps> = ({ isEditing = false }) => {
  ...
  const TOKEN_ENDPOINT_AUTH_METHOD_OPTIONS = [
    { value: "client_secret_basic", label: t("ui.Client Secret Basic") },
    { value: "client_secret_post", label: t("ui.Client Secret Post") },
  ];
  ...
  <Form.Item
    label={
      <span className="text-sm font-medium text-gray-700 flex items-center">
        ...
        <Tooltip title={t("ui.How the proxy authenticates to the upstream OAuth token endpoint. Client Secret Basic sends the client credentials in an HTTP Basic Authorization header; leave blank to use the default, Client Secret Post, which sends them in the request body.")}>
