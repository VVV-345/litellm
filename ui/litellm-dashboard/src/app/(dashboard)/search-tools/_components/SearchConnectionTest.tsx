    ...
    const runTest = async () => {
      ...
      try {
        ...
        if (result.status === "success") {
          toast.success(t("ui.Connection test successful!"));
        }
      } catch (error) {
        setTestResult({
          status: "error",
      ...
      <div className="rounded-lg bg-card p-6">
        <div className="flex flex-col items-center justify-center px-5 py-8">
          ...
          <p className="text-base text-foreground">
            {t("ui.Testing connection to")} {litellmParams.search_provider || "search provider"}...
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-center px-5 py-8">
          ...
          <div className="ml-3">
            <p className="text-lg font-medium text-success">
              {t("ui.Connection to")} {litellmParams.search_provider} {t("ui.successful!")}
            </p>
        ...
        <div>
          <div className="mb-5 flex items-center">
            ...
            <p className="text-lg font-medium text-destructive">
              {t("ui.Connection to")} {litellmParams.search_provider || "search provider"} {t("ui.failed")}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-warning/20 border-l-4 border-l-amber-500 bg-warning/10 p-4">
            <p className="mb-2 font-semibold text-warning">{t("ui.Troubleshooting tips:")}</p>
            <ul className="my-2 list-disc pl-5 text-warning">
              ...
              <li className="mb-1.5">{t("ui.Review the provider's documentation for any additional requirements")}</li>
            </ul>
          </div>
        </div>
      )}
