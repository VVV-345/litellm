...
const VectorStoreForm: React.FC<VectorStoreFormProps> = ({
  ...
  const handleCreate = async (formValues: VectorStoreFormValues) => {
    ...
    try {
      ...
      toast.success(t("ui.Vector store created successfully"));
  ...
  return (
    <Dialog open={isVisible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[1000px]">
        <DialogHeader>
          <DialogTitle>{t("ui.Add New Vector Store")}</DialogTitle>
        </DialogHeader>
        <TooltipProvider>
          <form onSubmit={form.handleSubmit(handleCreate)}>
            <FieldGroup>
              ...
              {selectedProvider === "pg_vector" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("ui.PG Vector Setup Required")}</AlertTitle>
                  <AlertDescription>
              ...
              {selectedProvider === "valkey" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("ui.Valkey Setup Required")}</AlertTitle>
                  <AlertDescription>
                    <p>
                      {t(
                        "ui.LiteLLM searches documents you have already stored in Valkey. It does not create the index or upload documents for you. Before creating this vector store, make sure:",
                      )}
                    </p>
                    <ol style={{ marginLeft: "16px", marginTop: "8px", listStyleType: "decimal" }}>
                      <li>
                        {t(
                          "ui.Your Valkey server has vector search enabled (the valkey-search module, included in the valkey-bundle image and in AWS ElastiCache / MemoryDB for Valkey)",
                        )}
                      </li>
                      <li>
                        {t(
                          "ui.You have already created a search index and loaded your documents and their embeddings into it. Enter that index name as the Vector Store ID",
                        )}
                      </li>
                      <li>
                        {t(
                          "ui.You know which embedding model created those stored embeddings. That model must be added to this proxy under Models so you can pick it below. Using a different model returns wrong results",
                        )}
                      </li>
                      <li>
                        {t(
                          'ui.You know the field names your documents use for their text and their embedding. If they are not "text" and "embedding", set them below',
                        )}
                      </li>
                    </ol>
                    <p style={{ marginTop: "8px" }}>
                      {t(
                        "ui.When a query comes in, LiteLLM converts it to an embedding with the model below and returns the closest matching documents from your index.",
                      )}
                    </p>
                  </AlertDescription>
                </Alert>
              ...
              {selectedProvider === "vertex_rag_engine" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("ui.Vertex AI RAG Engine Setup")}</AlertTitle>
                  <AlertDescription>
                    <p>{t("ui.To use Vertex AI RAG Engine:")}</p>
                    <p style={{ marginTop: "4px", fontStyle: "italic" }}>
                      {t(
                        'ui.Note: Google Cloud has renamed this to "RAG Engine" in its console — the steps below still apply.',
                      )}
                    </p>
                    <ol style={{ marginLeft: "16px", marginTop: "8px", listStyleType: "decimal" }}>
                      <li>
              ...
              {selectedProvider === "vertex_ai/search_api" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("ui.Vertex AI Search Setup")}</AlertTitle>
                  <AlertDescription>
                    ...
                    <ol style={{ marginLeft: "16px", marginTop: "8px", listStyleType: "decimal" }}>
                      <li>
                        {t(
                          "ui.Enable the Discovery Engine API on your Google Cloud project and create a data store following the guide:",
                        )}{" "}
                        <a
                      ...
                      <li>
                        {t(
                          "ui.For most data store types (Cloud Storage, BigQuery, Media): copy the data store ID and enter it in the Vector Store ID field below.",
                        )}
                      </li>
                      <li>
                        {t(
                          "ui.For website, healthcare, and connector-based sources (Drive, Gmail, Slack, Jira, etc.): create a search app on top of the data store, then copy the",
                        )}{" "}
                        <strong>{t("ui.Engine ID")}</strong>{" "}
                        {t(
                          "ui.and enter it in the Engine ID field. The Vector Store ID is still required as the LiteLLM-side name for this record, but it isn't used in the GCP URL when Engine ID is set.",
                        )}
                      </li>
                    </ol>
                  </AlertDescription>
      ...
      <FormField control={control} name={field.name} label={label}>
        {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
          ...
          >
            ...
            />
            <ComboboxContent>
              <ComboboxEmpty>{t("ui.No matching options")}</ComboboxEmpty>
              <ComboboxList>
                {(option: { value: string; label: string }) => (
