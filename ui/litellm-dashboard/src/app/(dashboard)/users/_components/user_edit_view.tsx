...
const budgetSchema = (unlimitedBudget: boolean, errorMessage: string) =>
  z.object({
    ...userEditShape,
    max_budget: z
...
export function UserEditView({
  ...
  const schema = useMemo(
    () => budgetSchema(unlimitedBudget, t("ui.Please enter a budget or select Unlimited Budget")),
    [unlimitedBudget, t],
  );
  ...
  return (
    <TooltipProvider>
      <form onSubmit={form.handleSubmit(handleSubmit)}>
        <FieldGroup>
          {!isBulkEdit && (
            <FormField control={form.control} name="user_id" label={t("ui.User ID")}>
              {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} disabled />}
            </FormField>
          )}

          {!isBulkEdit && (
            <FormField control={form.control} name="user_email" label={t("ui.Email")}>
              {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
            </FormField>
          )}

          <FormField control={form.control} name="user_alias" label={t("ui.User Alias")}>
            {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
          </FormField>
          ...
          <FormField
            ...
            label={labelWithHint(
              ...
              t("ui.This is the role that the user will globally on the proxy. This role is independent of any team/org specific roles."),
            )}
          >
            {({ id, value, onChange }) => (
              <Select
          ...
          <FormField
            ...
            label={labelWithHint(
              ...
              t("ui.Select which models this user can access outside of team-scope. Choose 'All Proxy Models' to grant access to all models available on the proxy."),
            )}
          >
            {({ value, onChange }) => (
              <MultiSelect
          ...
          {!isBulkEdit && (
            <ModelMaxBudgetField
              ...
              hint={t("ui.Cap this user's spend on individual models, each with its own reset window. Applies across every key the user holds.")}
            />
          )}
            ...
            <>
              <FormField
                ...
                label={labelWithHint(
                  ...
                  t("ui.Caps which MCP servers, access groups, and tools this user may reach. Every key the user holds is limited to this set."),
                )}
              >
                {({ value, onChange }) => (
                  <MCPServerSelector
