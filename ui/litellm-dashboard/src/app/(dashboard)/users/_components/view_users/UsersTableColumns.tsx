...
export const getUsersTableColumns = ({
  ...
  const baseColumns: ColumnDef<UserInfo>[] = [
        ...
        if (isScimInactive(row.original)) {
          return (
            <StatusBadge
              tone="error"
              label={t("ui.Inactive")}
              tooltip={t("ui.Deactivated via SCIM (external identity provider). The user's virtual keys are blocked.")}
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableSortHeader column={column} title={t("ui.Global Proxy Role")} variant="header-cycle" />,
      size: 160,
      enableSorting: true,
    ...
    },
    {
      ...
      size: 130,
      enableSorting: false,
      cell: ({ row }) => <MoneyCell value={row.original.max_budget} decimals={2} emptyText={t("ui.Unlimited")} showZero />,
    },
    {
      id: "sso_user_id",
      ...
      header: () => (
        <span className="flex items-center gap-1.5">
          {t("ui.SSO ID")}
          <CellTooltip
            content={t("ui.SSO ID is the ID of the user in the SSO provider. If the user is not using SSO, this will be null.")}
  ...
  return [
    createSelectionColumn<UserInfo>({
      rowAriaLabel: (row) => `${t("ui.Select")} ${row.original.user_email || row.original.user_id}`,
    }),
    ...baseColumns,
  ];
};
