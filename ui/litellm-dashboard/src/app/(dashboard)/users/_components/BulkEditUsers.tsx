  ...
  const handleSubmit = async (formValues: any) => {
    ...
    try {
      ...
      if (!hasUserUpdates && !hasTeamAdditions) {
        toast.fromError(t("ui.Please modify at least one field or select teams to add users to"));
        return;
      }
      ...
      if (hasUserUpdates) {
        if (updateAllUsers) {
          ...
          successMessages.push(t("ui.Updated all users ({{count}} total)", { count: result.total_requested }));
        } else {
          await userBulkUpdateUserCall(accessToken, updatePayload, userIds);
          successMessages.push(t("ui.Updated {{count}} user(s)", { count: userIds.length }));
        }
      }
      ...
      if (hasTeamAdditions) {
        ...
        if (successfulTeams.length > 0) {
          ...
          successMessages.push(t("ui.Added users to {{count}} team(s) ({{total}} total additions)", { count: successfulTeams.length, total: totalAdditions }));
        }

        if (failedTeams.length > 0) {
          toast.warning(t("ui.Failed to add users to {{count}} team(s)", { count: failedTeams.length }));
        }
      }
      ...
      toast.fromError(t("ui.Failed to perform bulk operations"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>
            {updateAllUsers ? t("ui.Bulk Edit All Users") : t("ui.Bulk Edit {{count}} User(s)", { count: selectedUsers.length })}
          </DialogTitle>
        </DialogHeader>
        {allowAllUsers && (
          <div className="mb-4">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                aria-label={t("ui.Update ALL users in the system")}
              />
              <label htmlFor={updateAllUsersId} className="cursor-pointer text-sm font-medium text-foreground">
                {t("ui.Update ALL users in the system")}
              </label>
            </div>
            {updateAllUsers && (
              <div className="mt-2">
                <span className="text-xs text-warning">
                  {t("ui.This will apply changes to ALL users in the system, not just the selected ones.")}
                </span>
              </div>
            )}
          </div>
        ...
        {!updateAllUsers && (
          <div className="mb-4">
            <h5 className="mb-2 text-sm font-semibold text-foreground">{t("ui.Selected Users ({{count}}):", { count: selectedUsers.length })}</h5>
            <div className="max-h-[200px] overflow-y-auto rounded-md border border-border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[30%]">{t("ui.User ID")}</TableHead>
                    <TableHead className="w-[25%]">{t("ui.Email")}</TableHead>
                    <TableHead className="w-[25%]">{t("ui.Current Role")}</TableHead>
                    <TableHead className="w-[20%]">{t("ui.Budget")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {selectedUsers.map((user) => (
                    <TableRow key={user.user_id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{user.user_email || t("ui.No email")}</TableCell>
                      <TableCell className="text-xs text-foreground">
                        {possibleUIRoles?.[user.user_role]?.ui_label || user.user_role}
                      </TableCell>
                      <TableCell>
                        <MoneyCell value={user.max_budget} decimals={2} emptyText={t("ui.Unlimited")} showZero />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <div className="mb-4">
          <p className="text-sm text-foreground">
            <strong>{t("ui.Instructions")}:</strong> {t("ui.Fill in the fields below with the values you want to apply to all selected users. You can bulk edit: role, budget, models, and metadata. You can also add users to teams.")}
          </p>
        </div>
        ...
        <Card size="sm" className="mb-4 bg-muted/50">
          <CardHeader>
            <CardTitle>{t("ui.Team Management")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  aria-label={t("ui.Add selected users to teams")}
                />
                <label htmlFor={addToTeamsId} className="cursor-pointer text-sm text-foreground">
                  {t("ui.Add selected users to teams")}
                </label>
              </div>
                ...
                <>
                  <div>
                    ...
                    <MultiSelect
                      id={selectedTeamsId}
                      className="mt-2"
                      placeholder={t("ui.Select teams to add users to")}
                      value={selectedTeams}
                  ...
                  <div>
                    <label htmlFor={teamBudgetId} className="block text-sm font-medium text-foreground">
                      {t("ui.Team Budget (Optional):")}
                    </label>
                    <NumericalInput
                      id={teamBudgetId}
                      className="mt-2"
                      placeholder={t("ui.Max budget per user in team")}
                      value={teamBudget ?? ""}
                    ...
                    <span className="text-xs text-muted-foreground">
                      {t("ui.Leave empty for unlimited budget within team limits")}
                    </span>
                  </div>

                  <span className="text-xs text-muted-foreground">
                    {t("ui.Users will be added with \"user\" role by default. All users will be added to each selected team.")}
                  </span>
                </>
              )}
            </div>
        ...
        {loading && (
          <div className="mt-2.5 text-center">
            <span className="text-sm text-foreground">
              {updateAllUsers ? t("ui.Updating all users...") : t("ui.Updating {{count}} user(s)...", { count: selectedUsers.length })}
            </span>
          </div>
        )}
      </DialogContent>
