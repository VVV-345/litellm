...
const getBudgetDurationOptions = (t: (key: string) => string) => [
  { value: NO_RESET, label: t("ui.No reset") },
  { value: "1h", label: t("ui.Hourly") },
  { value: "24h", label: t("ui.daily") },
  { value: "7d", label: t("ui.weekly") },
  { value: "30d", label: t("ui.monthly") },
] as const;

const getTeamRoleOptions = (t: (key: string) => string) => [
  { value: "user", label: t("ui.User") },
  { value: "admin", label: t("ui.Admin") },
] as const;

const getModelSentinelLabels = (t: (key: string) => string): ReadonlyMap<string, string> =>
  new Map(MODEL_SENTINEL_OPTIONS.map(({ value, label }) => [value, t(`ui.${label}`)]));
...
const TeamPickerField = ({ control, index }: { control: SettingsControl; index: number }) => {
  ...
  return (
    <FormField control={control} name={`teams.${index}.team_id`} label={t("ui.Team")}>
      {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
        <PaginatedSearchSelect
          ...
          placeholder={t("ui.Search a team")}
...
const TeamsField = ({ control }: { control: SettingsControl }) => {
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      <div>
        <p className="text-sm font-medium">{t("ui.Default Teams")}</p>
        <p className="text-sm text-muted-foreground">
          {t("ui.New users are added to these teams. Only teams that already exist can be selected.")}
        </p>
      </div>
      ...
      {fields.map((field, index) => (
        <div key={field.id} className="rounded-lg border border-border p-4">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-sm font-medium">{t("ui.Team")} {index + 1}</p>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            <FormField control={control} name={`teams.${index}.max_budget_in_team`} label={t("ui.Max Budget in Team (USD)")}>
              {({ ref, ...budgetField }) => (
                <Input {...budgetField} ref={ref} type="number" step="any" min={0} placeholder={t("ui.Optional")} />
              )}
            </FormField>

            <FormField control={control} name={`teams.${index}.user_role`} label={t("ui.Team Role")}>
              {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
                <Select
...
const SettingsView = ({ values, roleOptions }: SettingsViewProps) => {
  const { t } = useTranslation();
  const budgetDurationOptions = getBudgetDurationOptions(t);
  const modelSentinelLabels = getModelSentinelLabels(t);
  ...
  return (
    <div className="flex flex-col gap-4">
      <ViewRow label={t("ui.Default Role")}>{roleLabel === "" ? t("ui.Not set") : roleLabel}</ViewRow>
      <ViewRow label={t("ui.Max Budget (USD)")}>{values.max_budget === "" ? t("ui.Not set") : values.max_budget}</ViewRow>
      <ViewRow label={t("ui.Reset Budget")}>{durationLabel}</ViewRow>
      <ViewRow label={t("ui.Default Models")}>
      ...
      <div>
        ...
        {values.teams.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("ui.None")}</p>
        ) : (
          values.teams.map((team) => (
            <p key={team.team_id} className="text-sm text-muted-foreground">
              {team.team_id}
              {team.max_budget_in_team !== "" && (
                <> · ${team.max_budget_in_team} {t("ui.Max Budget")}</>
              )}
              <> · {team.user_role === "admin" ? t("ui.Admin") : t("ui.User")}</>
            </p>
          ))
        )}
      </div>
...
const SettingsForm = ({ initialValues, roleOptions, updateSettings, onCancel, onSaved }: SettingsFormProps) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_result, values) => {
      toast.success(t("ui.Default user settings updated successfully"));
      queryClient.invalidateQueries({ queryKey: SETTINGS_QUERY_KEY });
      form.reset(values);
      onSaved();
    },
    onError: (error: unknown) =>
      toast.fromError(error instanceof Error ? error.message : t("ui.Failed to update default user settings")),
  });
    ...
    <form onSubmit={onSubmit} noValidate>
      <FieldGroup>
        <FormField
          ...
          name="user_role"
          label={t("ui.Default Role")}
          description={t("ui.Role assigned to new users")}
        >
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            <Select
              items={roleOptions}
            ...
            >
              <SelectTrigger id={id} className="w-full" aria-invalid={ariaInvalid} aria-describedby={ariaDescribedBy}>
                <SelectValue placeholder={t("ui.Not set")} />
              </SelectTrigger>
              <SelectContent>
        ...
        <FormField
          control={form.control}
          name="max_budget"
          label={t("ui.Max Budget (USD)")}
          description={t("ui.Default maximum budget for new users")}
        >
        ...
        <FormField
          ...
          label={t("ui.Reset Budget")}
          description={t("ui.How often the default budget resets")}
        >
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            <Select
        ...
        <FormField
          control={form.control}
          name="models"
          label={t("ui.Default Models")}
          description={t("ui.Models new users can access")}
        >
          {(field) => (
            <ModelSelect
              value={field.value}
...
const SettingsCard = ({ action, children }: { action?: React.ReactNode; children: React.ReactNode }) => {
  ...
  <Card>
    <CardHeader>
      ...
      <CardDescription>
        {t("ui.Applied to every new internal user created through SSO or the user management APIs.")}
      </CardDescription>
...
export const DefaultUserSettingsForm = ({
  ...
  if (isError || initialValues === undefined) {
    return (
      <SettingsCard>
        <p role="alert">{t("ui.Could not load the default user settings.")}</p>
      </SettingsCard>
    );
  }
