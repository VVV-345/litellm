...
-d '{
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-gray-900 mb-1">{t("ui.Proxy base URL")}</h3>
        <p className="text-sm text-gray-600 font-mono bg-gray-50 px-2 py-1.5 rounded-sm border border-gray-200 break-all">
          {baseUrl}
        </p>
      </div>
      <div>
        <h3 className="text-sm font-semibold text-gray-900 mb-2">{t("ui.Call your agent (cURL)")}</h3>
        <CodeBlock code={curlExample} language="bash" />
      </div>
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
        <h3 className="text-sm font-semibold text-gray-900 mb-2">{t("ui.Create a key for this agent")}</h3>
        <p className="text-sm text-gray-600 mb-3">
          {t(
            "ui.Create a virtual key that can only call this agent. The key will be scoped to you (user_id) and restricted to",
          )}{" "}
        ...
        <Button onClick={onCreateKey} disabled={creatingKey || disabledPersonalKeyCreation}>
          {t("ui.Create key for this agent")}
        </Button>
        {disabledPersonalKeyCreation && (
          <p className="text-xs text-amber-600 mt-2">{t("ui.Key creation is disabled for your account.")}</p>
        )}
        {createdKeyValue && (
          <p className="text-xs text-green-700 mt-2">
            {t("ui.Key created. It is shown in the cURL example above — copy the snippet to use it.")}
          </p>
        )}
      </div>
    </div>
...
export default function AgentBuilderView({
  ...
  const loadAgents = useCallback(async (): Promise<AgentModel[]> => {
    ...
    try {
      ...
      console.error(e);
      NotificationsManager.fromBackend(t("ui.Failed to load agents"));
      return [];
    } finally {
  ...
  const handleSaveAgent = async () => {
    if (!accessToken || !draftName?.trim() || !draftUnderlyingModel) {
      NotificationsManager.fromBackend(t("ui.Name and underlying model are required"));
      return;
    }
    setSaving(true);
    try {
      ...
      NotificationsManager.fromBackend(t("ui.Failed to save agent"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateAgent = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("ui.Agent updated successfully"));
      const list = await loadAgents();
      ...
      NotificationsManager.fromBackend(t("ui.Failed to update agent"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateKeyForAgent = async () => {
    ...
    try {
      ...
      if (keyValue) {
        ...
        NotificationsManager.success(t("ui.Virtual key created. Use it in the curl example below."));
      } else {
        NotificationsManager.fromBackend(t("ui.Key created but value not returned"));
      }
    } catch (e) {
      NotificationsManager.fromBackend(t("ui.Failed to create key for agent"));
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      NotificationsManager.success(t("ui.Agent deleted"));
      const list = await loadAgents();
  ...
  if (!accessToken || !userID || !userRole) {
    return (
      <div className="flex h-full items-center justify-center p-8 text-gray-500">
        {t("ui.Sign in to use Agent Builder.")}
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col bg-white text-gray-900">
      <div className="flex shrink-0 flex-col border-b border-gray-200">
        <div className="flex h-12 items-center justify-between px-4">
          <span className="text-sm font-medium text-gray-900">{t("ui.Agent Builder")}</span>
          {isNewAgent ? (
            ...
            <span className="text-xs text-gray-500">
              {t("ui.Build Agents that pass your compliance requirements.")}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 border-t border-amber-200 bg-amber-50 px-4 py-2 text-xs text-amber-800">
          ...
          <span>
            {t("ui.Agent Builder is experimental and may change or be removed without notice. We'd love your feedback—email us at")}{" "}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-60 shrink-0 border-r border-gray-200 bg-white flex flex-col">
          <div className="flex items-center justify-between border-b border-gray-200 p-3">
            <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">{t("ui.Agents")}</span>
            <Button variant="ghost" size="icon-sm" onClick={handleAddAgent} aria-label={t("ui.Add agent")}>
              <Plus />
            </Button>
          </div>
          <div className="flex-1 overflow-y-auto p-2">
            {loadingAgents ? (
              ...
              <>
                ...
                >
                  <Plus className="mr-1 inline size-4" /> {t("ui.New agent")}
                </button>
              </>
            )}
          </div>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          {selectedId === null && !isNewAgent && agentModels.length === 0 && !loadingAgents && (
            <div className="flex flex-1 items-center justify-center p-8 text-gray-500">
              {t("ui.No agents yet. Add an agent to get started.")}
            </div>
          )}
                      ...
                      <div className="mx-auto max-w-xl space-y-4">
                        {!selectedAgentModelId && selectedAgent && (
                          <div className="rounded-sm border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                            {t(
                              "ui.This agent cannot be updated or deleted here (missing model id). Manage it from Models & Endpoints.",
                            )}
                          </div>
                        )}
                        <div>
                          <label className="mb-1 block text-sm font-medium text-gray-700">{t("ui.Agent name")}</label>
                          <Input
                            value={draftName}
                            onChange={(e) => setDraftName(e.target.value)}
                            placeholder={t("ui.My Agent")}
                          />
                        </div>
                        <div>
                          ...
                          <Textarea
                            ...
                            placeholder={t("ui.You are a helpful assistant...")}
                            rows={6}
                        ...
                        <div>
                          ...
                          >
                            <SelectTrigger className="w-full" aria-label={t("ui.Underlying LLM")}>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            <label className="mb-1 block text-sm font-medium text-gray-700">{t("ui.Max tokens")}</label>
                            <Input
                              type="number"
                              min={1}
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-medium text-gray-700">{t("ui.MCP servers")}</label>
                          <MultiSelect
                            placeholder={t(
                              "ui.Select MCP servers to attach (same format as chat completions API)",
                            )}
                  ...
                  <div className="flex h-full flex-col min-h-0">
                    {selectedAgent ? (
                      ...
                      <div className="flex flex-1 items-center justify-center text-gray-500">
                        {t("ui.Save an agent first to test in Chat.")}
                      </div>
                    )}
                  </div>
                </TabsContent>
                <TabsContent value="test" keepMounted={hasVisited("test")} className="min-h-0 overflow-hidden">
                  <div className="flex h-full flex-col min-h-0">
                    {selectedAgent ? (
                      ...
                      <div className="flex flex-1 items-center justify-center text-gray-500">
                        {t("ui.Select an agent to run batch tests.")}
                      </div>
                    )}
                  </div>
                </TabsContent>
                <TabsContent value="connect" keepMounted={hasVisited("connect")} className="min-h-0 overflow-hidden">
                  <div className="h-full overflow-y-auto p-6">
                    {selectedAgent ? (
                      ...
                      <div className="flex flex-1 items-center justify-center text-gray-500">
                        {t("ui.Select an agent to see how to connect.")}
                      </div>
                    )}
                  </div>
                </TabsContent>
      ...
      <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("ui.Delete agent")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "ui.Are you sure you want to delete",
              )}{" "}
              &quot;{selectedAgent?.model_name}&quot;?{" "}
              {t("ui.This cannot be undone.")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction variant="outline">{t("ui.Cancel")}</AlertDialogAction>
