...
-d '{
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-1">{t("ui.Proxy base URL")}</h3>
        <p className="text-sm text-muted-foreground font-mono bg-muted px-2 py-1.5 rounded-sm border border-border break-all">
          {baseUrl}
        </p>
      </div>
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-2">{t("ui.Call your agent (cURL)")}</h3>
        <CodeBlock code={curlExample} language="bash" />
      </div>
      <div className="rounded-lg border border-border bg-muted p-4">
        <h3 className="text-sm font-semibold text-foreground mb-2">{t("ui.Create a key for this agent")}</h3>
        <p className="text-sm text-muted-foreground mb-3">
          {t(
            "ui.Create a virtual key that can only call this agent. The key will be scoped to you (user_id) and restricted to the model",
          )}{" "}
        ...
        {disabledPersonalKeyCreation && (
          <p className="text-xs text-warning mt-2">{t("ui.Key creation is disabled for your account.")}</p>
        )}
        {createdKeyValue && (
...
export default function AgentBuilderView({
  ...
  const loadAgents = useCallback(async (): Promise<AgentModel[]> => {
    ...
    try {
      ...
      console.error(e);
      toast.fromError(t("ui.Failed to load agents"));
      return [];
    } finally {
  ...
  const handleSaveAgent = async () => {
    if (!accessToken || !draftName?.trim() || !draftUnderlyingModel) {
      toast.fromError(t("ui.Name and underlying model are required"));
      return;
    }
    setSaving(true);
    try {
      ...
      toast.fromError(t("ui.Failed to save agent"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateAgent = async () => {
    ...
    try {
      ...
      toast.fromError(t("ui.Failed to update agent"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateKeyForAgent = async () => {
    ...
    try {
      ...
      if (keyValue) {
        ...
        toast.success(t("ui.Virtual key created. Use it in the curl example below."));
      } else {
        toast.fromError(t("ui.Key created but value not returned"));
      }
    } catch (e) {
      toast.fromError(t("ui.Failed to create key for agent"));
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("ui.Agent deleted"));
      const list = await loadAgents();
      ...
      toast.fromError(t("ui.Failed to delete agent"));
    } finally {
      setDeleting(false);
    ...
    <div className="flex h-full flex-col bg-card text-foreground">
      <div className="flex shrink-0 flex-col border-b border-border">
        <div className="flex h-12 items-center justify-between px-4">
          <span className="text-sm font-medium text-foreground">{t("ui.Agent Builder")}</span>
          {isNewAgent ? (
        ...
        <div className="flex items-center gap-2 border-t border-warning/20 bg-warning/10 px-4 py-2 text-xs text-warning">
          ...
          <span>
            {t(
              "ui.Agent Builder is experimental and may change or be removed without notice. We’d love your feedback—email us at",
            )}{" "}
                  ...
                  <div className="h-full overflow-y-auto p-6">
                    {isNewAgent || selectedAgent ? (
                      <div className="mx-auto max-w-xl space-y-4">
                        ...
                        <div>
                          <label className="mb-1 block text-sm font-medium text-foreground">{t("ui.Agent name")}</label>
                          <Input
                            value={draftName}
                        ...
                        <div>
                          ...
                          <MultiSelect
                            placeholder={t("ui.Select MCP servers to attach (same format as chat completions API)")}
                          ...
                          {selectedAgent && draftTools.length > 0 && (
                            <p className="mt-1 text-xs text-muted-foreground">
                              {draftTools.length} {t("ui.MCP server")}
                              {draftTools.length !== 1 ? "s" : ""} {t("ui.saved. Use the same")}{" "}
                              <code className="rounded-sm bg-muted px-1">{t("ui.tools")}</code>{" "}
                              {t("ui.array in chat completions when calling this agent.")}
                            </p>
                          )}
                        </div>
                        {selectedAgent && (
      ...
      <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("ui.Are you sure you want to delete agent \"{{name}}\"? This cannot be undone.", {
