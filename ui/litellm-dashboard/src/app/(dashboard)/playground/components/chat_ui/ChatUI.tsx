...
const ChatUI: React.FC<ChatUIProps> = ({
  ...
  const mcpServerOptions = useMemo((): MultiSelectOption[] => {
    ...
    if (endpointType !== EndpointType.MCP) {
      options.push({
        value: "__all__",
        label: t("ui.All MCP Servers"),
        description: t("ui.Use all available MCP servers"),
      });
    }
  ...
  const handleSendMessage = async () => {
    ...
    if (endpointType === EndpointType.IMAGE_EDITS && uploadedImages.length === 0) {
      NotificationsManager.fromBackend(t("ui.Please upload at least one image for editing"));
      return;
    }

    // For audio transcriptions, require audio file
    if (endpointType === EndpointType.TRANSCRIPTION && !uploadedAudio) {
      NotificationsManager.fromBackend(t("ui.Please upload an audio file for transcription"));
      return;
    }

    // For A2A agents, require agent selection
    if (endpointType === EndpointType.A2A_AGENTS && !selectedAgent) {
      NotificationsManager.fromBackend(t("ui.Please select an agent to send a message"));
      return;
    }
    ...
    if (endpointType === EndpointType.MCP) {
      ...
      if (!rawSelected) {
        NotificationsManager.fromBackend(t("ui.Please select an MCP server to test"));
        return;
      }
      if (!selectedMCPDirectTool) {
        NotificationsManager.fromBackend(t("ui.Please select an MCP tool to call"));
        return;
      }
      ...
      if (!mcpTool) {
        NotificationsManager.fromBackend(t("ui.Please wait for tool schema to load"));
        return;
      }
      try {
        ...
        NotificationsManager.fromBackend(
          err instanceof Error ? err.message : t("ui.Please fill in all required parameters"),
        );
        return;
      }
    }
    ...
    if (modelRequiredEndpoints.includes(endpointType as EndpointType) && !selectedModel) {
      NotificationsManager.fromBackend(t("ui.Please select a model before sending a request"));
      return;
    }
    ...
    if (!effectiveApiKey) {
      NotificationsManager.fromBackend(t("ui.Please provide a Virtual Key or select Current UI Session"));
      return;
    }
    ...
    if (endpointType === EndpointType.RESPONSES && responsesUploadedImage) {
      try {
        ...
        NotificationsManager.fromBackend(t("ui.Failed to process image. Please try again."));
        return;
      }
    }
    ...
    try {
      ...
      if (endpointType === EndpointType.MCP) {
        ...
        if (resolvedServerId && !resolvedServerId.startsWith("toolset:") && selectedMCPDirectTool) {
          ...
          updateTextUI("assistant", resultText || t("ui.Tool executed successfully."));
        }
      }
  ...
  let modelEmptyText = t("ui.No models available for this key");
  if (modelLoadError) {
    modelEmptyText = t("ui.Unable to load models for this key");
  } else if (apiKeySource === "custom" && !apiKey.trim()) {
    modelEmptyText = t("ui.Enter a Virtual Key to load models");
  } else if (modelInfo.length > 0 && modelsForEndpoint.length === 0) {
    modelEmptyText = t("ui.No models available for this endpoint");
  }
    ...
    <div className={`min-h-0 min-w-0 bg-white ${simplified ? "flex h-full w-full flex-col" : "h-full w-full p-3"}`}>
        ...
        <div className="flex h-full min-h-0 min-w-0 w-full flex-col lg:flex-row">
          {!simplified && (
            <div className="max-h-[42%] w-full shrink-0 overflow-y-auto border-b border-gray-200 bg-gray-50 p-4 lg:max-h-none lg:w-72 lg:border-r lg:border-b-0 xl:w-80">
              <h2 className="mb-6 mt-2 text-xl font-semibold">{t("ui.Configurations")}</h2>
              <div className="space-y-4">
                <div>
                  <label className="mb-2 flex items-center text-sm font-medium text-gray-700">
                    <Key className="mr-2 size-4" aria-hidden="true" /> {t("ui.Virtual Key Source")}
                  </label>
                  <ShadcnSelect
                  ...
                  >
                    <SelectTrigger className="w-full" size="sm" aria-label={t("ui.Virtual Key Source")}>
                      <SelectValue>
                        {apiKeySource === "custom" ? t("ui.Virtual Key") : t("ui.Current UI Session")}
                      </SelectValue>
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="session">{t("ui.Current UI Session")}</SelectItem>
                  ...
                  {apiKeySource === "custom" && (
                    <div className="relative mt-2">
                      ...
                      <Input
                        className="h-8 pl-8"
                        placeholder={t("ui.Enter custom Virtual Key")}
                        type="password"
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <label className="flex items-center text-sm font-medium text-gray-700">
                      <Settings className="mr-2 size-4" aria-hidden="true" /> {t("ui.Custom Proxy Base URL")}
                    </label>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      className="h-8 pl-8"
                      placeholder={t("ui.Optional: Enter custom proxy URL (e.g., http://localhost:5000)")}
                ...
                <div>
                  <label className="mb-2 flex items-center text-sm font-medium text-gray-700">
                    <Wrench className="mr-2 size-4" aria-hidden="true" /> {t("ui.Endpoint Type")}
                  </label>
                  <EndpointSelector
                  ...
                  {endpointType === EndpointType.SPEECH && (
                    <div className="mb-4">
                      ...
                      <ShadcnSelect value={selectedVoice} onValueChange={handleVoiceChange}>
                        <SelectTrigger className="w-full" size="sm" aria-label={t("ui.Voice")}>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between text-sm font-medium text-gray-700">
                      <span className="flex items-center">
                        <Bot className="mr-2 size-4" aria-hidden="true" /> {t("ui.Select Model")}
                      </span>
                      {isChatModel() || supportsStreamingToggle ? (
                          ...
                          <PopoverTrigger
                            render={
                              <Button
                                ...
                                aria-label={t("ui.Model Settings")}
                        ...
                        <Tooltip>
                          <TooltipTrigger
                            render={
                              <Button
                                ...
                                disabled
                                aria-label={t("ui.Model Settings unavailable")}
                              />
                            }
                          >
                            <Settings className="size-3.5" />
                          ...
                          <TooltipContent>
                            {t("ui.Advanced parameters are only supported for chat models currently")}
                          </TooltipContent>
                        </Tooltip>
                      )}
                    </div>
                    <SearchSelect
                      value={selectedModel}
                      placeholder={isLoadingModels ? t("ui.Loading models...") : t("ui.Select a Model")}
                      emptyText={modelEmptyText}
                      disabled={isLoadingModels}
                      onValueChange={onModelChange}
                      options={[
                        { value: "custom", label: t("ui.Enter custom model") },
                    ...
                    {showCustomModelInput && (
                      <Input
                        className="mt-2 h-8"
                        placeholder={t("ui.Enter custom model name")}
                ...
                {endpointType === EndpointType.A2A_AGENTS && (
                  <div>
                    <label className="mb-2 flex items-center text-sm font-medium text-gray-700">
                      <Bot className="mr-2 size-4" aria-hidden="true" /> {t("ui.Select Agent")}
                    </label>
                    <SearchSelect
                      value={selectedAgent}
                    ...
                    {agentInfo.length === 0 && (
                      <p className="mt-2 text-xs text-gray-500">
                        {t("ui.No agents found. Create agents via /v1/agents endpoint.")}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div>
                  <label className="mb-2 flex items-center text-sm font-medium text-gray-700">
                    <Tags className="mr-2 size-4" aria-hidden="true" /> {t("ui.Tags")}
                  </label>
                  <TagSelector
                    value={selectedTags}
                ...
                <div>
                  <div className="mb-2 flex items-center gap-1 text-sm font-medium text-gray-700">
                    ...
                    {endpointType === EndpointType.MCP ? t("ui.MCP Server") : t("ui.MCP Servers")}
                    <Tooltip>
                      <TooltipTrigger
                        render={
                          <button
                            ...
                            aria-label={t("ui.About MCP servers and toolsets")}
                      ...
                      <TooltipContent className="max-w-xs">
                        {endpointType === EndpointType.MCP
                          ? t("ui.Select an MCP server or toolset to test tools directly.")
                          : t("ui.Select MCP servers or toolsets to use in your conversation.")}
                      </TooltipContent>
                    </Tooltip>
                  </div>
                  {endpointType === EndpointType.MCP ? (
                    <SearchSelect
                      ...
                      placeholder={t("ui.Select MCP server")}
                      emptyText={isLoadingMCPServers ? t("ui.Loading...") : t("ui.No MCP servers")}
                  ...
                  {endpointType === EndpointType.MCP &&
                    ...
                    (() => {
                      ...
                      return (
                        <div className="mt-3">
                          <p className="mb-1 block text-xs text-gray-600">{t("ui.Select Tool")}</p>
                          <SearchSelect
                            value={selectedMCPDirectTool}
                            placeholder={t("ui.Select a tool to call")}
                          ...
                          return (
                            <div key={serverId} className="rounded-sm border p-2">
                              ...
                              <MultiSelect
                                ...
                                placeholder={t("ui.All tools (default)")}
                              ...
                              {server.has_user_credential ? (
                                <div className="flex items-center gap-2">
                                  <span className="flex items-center gap-1 text-xs font-medium text-green-600">
                                    <Key className="size-3" /> {t("ui.Connected")}
                                  </span>
                                  <button
                                    type="button"
                ...
                <div>
                  <div className="mb-2 flex items-center gap-1 text-sm font-medium text-gray-700">
                    <Database className="mr-1 size-4" aria-hidden="true" /> {t("ui.Vector Store")}
                    <Tooltip>
                      <TooltipTrigger aria-label={t("ui.About vector stores")}>
                        <Info className="size-3.5 text-gray-400" />
                      </TooltipTrigger>
                      <TooltipContent className="max-w-xs">
                        {t(
                          "ui.Select vector store(s) to use for this LLM API call. You can set up your vector store",
                        )}{" "}
                ...
                <div>
                  <div className="mb-2 flex items-center gap-1 text-sm font-medium text-gray-700">
                    <Shield className="mr-1 size-4" aria-hidden="true" /> {t("ui.Guardrails")}
                    <Tooltip>
                      <TooltipTrigger aria-label={t("ui.About guardrails")}>
                        <Info className="size-3.5 text-gray-400" />
                      </TooltipTrigger>
                      <TooltipContent className="max-w-xs">
                        {t("ui.Select guardrail(s) to use for this LLM API call. You can set up your guardrails")}{" "}
                ...
                {canViewPolicies && (
                  <div>
                    <div className="mb-2 flex items-center gap-1 text-sm font-medium text-gray-700">
                      <Shield className="mr-1 size-4" aria-hidden="true" /> {t("ui.Policies")}
                      <Tooltip>
                        <TooltipTrigger aria-label={t("ui.About policies")}>
                          <Info className="size-3.5 text-gray-400" />
                        </TooltipTrigger>
                        <TooltipContent className="max-w-xs">
                          {t(
                            "ui.Select policy/policies to apply to this LLM API call. Policies define which guardrails are applied based on conditions. You can set up your policies",
                          )}{" "}
            ...
            {endpointType === EndpointType.REALTIME ? (
              ...
              <>
                <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-gray-200 p-3 sm:p-4">
                  <h2 className="mb-0 text-xl font-semibold">{simplified ? t("ui.Chat") : t("ui.Test Key")}</h2>
                ...
                <div className="min-h-0 min-w-0 flex-1 overflow-auto p-3 pb-0 sm:p-4 sm:pb-0">
                  {chatHistory.length === 0 && (
                    <div className="flex h-full flex-col items-center justify-center text-gray-400">
                      ...
                      <p className="text-sm">{t("ui.Start a conversation, generate an image, or handle audio")}</p>
                    </div>
                  )}
                      ...
                      <div className="mb-4 text-left">
                        ...
                        >
                          <div className="mb-1.5 flex items-center gap-2">
                            ...
                            <strong className="text-sm capitalize">{t("ui.Assistant")}</strong>
                          </div>
                  ...
                  {isLoading && (
                    <div className="my-4 flex items-center justify-center">
                      <Loader2 className="size-6 animate-spin text-gray-500" aria-label={t("ui.Loading")} />
                    </div>
                  )}
                ...
                <div className="max-h-[50%] shrink-0 overflow-y-auto border-t border-gray-200 bg-white p-3 sm:p-4">
                    ...
                    <div className="mb-4">
                      {uploadedImages.length === 0 ? (
                        ...
                        >
                          ...
                          <p className="text-sm">{t("ui.Click or drag images to upload")}</p>
                          <p className="text-xs text-gray-500">
                            {t("ui.Support for PNG, JPG, JPEG, GIF, WebP. Multiple images supported.")}
                          </p>
                          <input
                            type="file"
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <label className="flex h-32 w-32 cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-gray-300 hover:border-gray-400">
                            ...
                            <p className="mt-1 text-xs text-gray-500">{t("ui.Add more")}</p>
                            <input
                              type="file"
                              accept={IMAGE_EDIT_ACCEPT}
                              multiple
                    ...
                    <div className="mb-4">
                      {!uploadedAudio ? (
                        ...
                        >
                          ...
                          <p className="text-sm">{t("ui.Click or drag audio file to upload")}</p>
                          <p className="text-xs text-gray-500">
                            {t(
                              "ui.Support for MP3, MP4, MPEG, MPGA, M4A, WAV, WEBM formats. Max file size: 25 MB.",
                            )}
                          </p>
                          <input
                            type="file"
                  ...
                  {endpointType === EndpointType.RESPONSES && codeInterpreter.enabled && (
                    <div className="mb-2 space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          {isLoading ? (
                            <>
                              ...
                              <span className="text-sm font-medium text-blue-700">{t("ui.Running Python code...")}</span>
                            </>
                          ) : (
                            <>
                              <Code2 className="size-4 text-blue-500" aria-hidden="true" />
                              <span className="text-sm font-medium text-blue-700">{t("ui.Code Interpreter Active")}</span>
                            </>
                          )}
                        </div>
                        <button
                      ...
                      {!isLoading && (
                        <div className="flex flex-wrap gap-2">
                          {[
                            t("ui.Generate sample sales data CSV and create a chart"),
                            t("ui.Create a PNG bar chart comparing AI gateway providers including LiteLLM"),
                            t("ui.Generate a CSV of LLM pricing data and visualize it as a line chart"),
                          ].map((prompt, idx) => (
                            <button
                              key={idx}
                              type="button"
                  ...
                  <ChatComposer
                    ...
                    suggestions={
                      endpointType === EndpointType.A2A_AGENTS
                        ? [
                            t("ui.What can you help me with?"),
                            t("ui.Tell me about yourself"),
                            t("ui.What tasks can you perform?"),
                          ]
                        : [
                            t("ui.Write me a poem"),
                            t("ui.Explain quantum computing"),
                            t("ui.Draft a polite email requesting a meeting"),
                          ]
                    }
                          ...
                          <CodeInterpreterToggle
                            ...
                            onToggle={() => {
                              ...
                              if (!codeInterpreter.enabled) {
                                NotificationsManager.success(t("ui.Code Interpreter enabled!"));
                              }
                            }}
                          />
                        )}
      ...
      <Dialog open={isGetCodeModalVisible} onOpenChange={setIsGetCodeModalVisible}>
        <DialogContent className="sm:max-w-3xl">
          ...
          <div className="my-2 flex items-end justify-between gap-3">
            <div>
              <p className="mb-1 text-sm font-medium text-gray-700">{t("ui.SDK Type")}</p>
              <ShadcnSelect value={selectedSdk} onValueChange={(value) => setSelectedSdk(value as "openai" | "azure")}>
                <SelectTrigger className="w-[150px]" size="sm" aria-label={t("ui.SDK Type")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="openai">{t("ui.OpenAI SDK")}</SelectItem>
                  <SelectItem value="azure">{t("ui.Azure SDK")}</SelectItem>
                </SelectContent>
              </ShadcnSelect>
            </div>
            <Button
              ...
              onClick={() => {
                void navigator.clipboard.writeText(generatedCode).then(
                  ...
                  () => NotificationsManager.error(t("ui.Unable to copy to clipboard")),
                );
              }}
            >
      ...
      <Dialog open={isToolsetsInfoModalVisible} onOpenChange={setIsToolsetsInfoModalVisible}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("ui.How Toolsets Work")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <p className="text-gray-700">
              <strong>{t("ui.Toolsets")}</strong>{" "}
              {t(
                "ui.are named collections of specific tools from one or more MCP servers. Instead of exposing all tools from a server, a toolset gives an agent exactly the tools it needs.",
              )}
            </p>
            <div>
              <h4 className="mb-2 font-semibold text-gray-800">{t("ui.How to use a toolset:")}</h4>
              <ol className="list-inside list-decimal space-y-2 text-gray-700">
                <li>
                  {t("ui.Select a")}{" "}
                  <span className="font-semibold text-violet-600">{t("ui.Toolset")}</span>{" "}
                  {t("ui.(purple badge) from the MCP Servers dropdown.")}
                </li>
                <li>{t("ui.The tool picker will show only the tools included in that toolset.")}</li>
                <li>{t("ui.Select a tool and fill in its parameters, then send.")}</li>
                <li>{t("ui.The tool call is routed to the correct underlying MCP server automatically.")}</li>
              </ol>
            </div>
            <div className="rounded-sm border border-purple-200 bg-purple-50 p-3">
              <p className="text-sm text-purple-800">
                <strong>{t("ui.Example:")}</strong>{" "}
                {t(
                  "ui.A GitHub Read-only toolset might include only",
                )}{" "}
                <code>list_repos</code> {t("ui.and")} <code>get_file</code>{" "}
                {t("ui.from a GitHub MCP server, preventing agents from making writes.")}
              </p>
            </div>
            <div>
              <h4 className="mb-1 font-semibold text-gray-800">{t("ui.Creating toolsets:")}</h4>
              <p className="text-sm text-gray-600">
                {t("ui.Admins can create and manage toolsets from the")}{" "}
                <strong>{t("ui.MCP")}</strong> {t("ui.page")} <strong>{t("ui.Toolsets")}</strong>{" "}
                {t("ui.tab. Toolsets can then be assigned to keys and teams to scope their tool access.")}
              </p>
            </div>
          </div>
          <DialogFooter>
