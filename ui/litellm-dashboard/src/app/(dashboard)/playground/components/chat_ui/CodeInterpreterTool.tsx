...
const CodeInterpreterTool: React.FC<CodeInterpreterToolProps> = ({
  ...
  const handleToggle = (checked: boolean) => {
    if (checked && !isOpenAI) {
      toast.warning(t("ui.Code Interpreter is only available for OpenAI models"));
      return;
    }
    ...
    <div className="border border-border rounded-lg p-3 bg-linear-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-foreground">{t("ui.Code Interpreter")}</span>
          <Tooltip>
            <TooltipTrigger aria-label={t("ui.About Code Interpreter")}>
              <Info className="size-3 text-muted-foreground" />
            </TooltipTrigger>
            <TooltipContent>
              {t("ui.Run Python code to generate files, charts, and analyze data. Container is created automatically.")}
            </TooltipContent>
          </Tooltip>
        </div>
        <Switch
