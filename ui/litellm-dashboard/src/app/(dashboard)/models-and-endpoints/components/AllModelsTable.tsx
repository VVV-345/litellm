...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-base font-semibold text-foreground">{translateUiText(t, "No models found")}</div>
      <div className="max-w-80 text-sm text-muted-foreground">
        {translateUiText(t, "No models match your search or filters. Try resetting them.")}
      </div>
    </div>
  );
}
...
export function AllModelsTable({
  ...
  const modelGroupOptions = useMemo(
    () => [
      { label: ui("All Models"), value: ALL_MODEL_GROUPS_VALUE },
      { label: ui("Wildcard Models (*)"), value: WILDCARD_MODEL_GROUP_VALUE },
      ...availableModelGroups.map((group) => ({ label: group, value: group })),
    ],
    [availableModelGroups, ui],
  );

  const accessGroupOptions = useMemo(
    () => [
      { label: ui("All Model Access Groups"), value: ALL_MODEL_GROUPS_VALUE },
      ...availableModelAccessGroups.map((accessGroup) => ({ label: accessGroup, value: accessGroup })),
    ],
    [availableModelAccessGroups, ui],
  );
  ...
  const filterLabels = Object.fromEntries(Object.entries(FILTER_LABEL_KEYS).map(([key, value]) => [key, ui(value)]));
  const viewModeLabels = Object.fromEntries(
    Object.entries(VIEW_MODE_LABEL_KEYS).map(([key, value]) => [key, ui(value)]),
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={ui("Loading models…")}
      ...
      toolbar={(table) => (
        <>
          <DataTableToolbar
            ...
            searchPlaceholder={ui("Search model names…")}
          ...
          >
            <Select value={selectedTeamValue} onValueChange={(value) => onTeamChange(String(value))}>
              ...
              >
                ...
                />
                <span className="text-muted-foreground">{ui("Team")}</span>
            ...
            <Select value={viewMode} onValueChange={(value) => onViewModeChange(value as ModelViewMode)}>
              <SelectTrigger size="sm" aria-label={ui("View")} data-testid="models-view-select" className="gap-2">
                <span className="text-muted-foreground">{ui("View")}</span>
            ...
            <Button
              variant="outline"
              size="icon-sm"
              aria-label={ui("Model Settings")}
              title={ui("Model Settings")}
          ...
          <DataTableFilterDrawer
            ...
            title={ui("Filters")}
            description={ui("Narrow down models + endpoints")}
            resetLabel={ui("Reset Filters")}
            onReset={onResetFilters}
          >
            {({ get, set }) => (
              <>
                <DataTableFilterField label={ui("Public Model Name")}>
                  <SearchSelect
                    ...
                    placeholder={ui("Filter by Public Model Name")}
                    emptyText={ui("No models found")}
                  />
                </DataTableFilterField>
                <DataTableFilterField label={ui("Model Access Group")}>
                  <SearchSelect
                    ...
                    placeholder={ui("Filter by Model Access Group")}
                    emptyText={ui("No model access groups found")}
                  />
                </DataTableFilterField>
              </>
            )}
