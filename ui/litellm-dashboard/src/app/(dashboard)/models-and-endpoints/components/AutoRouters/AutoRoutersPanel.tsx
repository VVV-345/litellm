      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {t(
              "ui.Auto routers sit above your deployments and pick a model per request. They are called like any other model, so clients keep using a single model name.",
            )}
          </p>
        </div>
        {canCreate && (
      ...
      <Dialog open={isCreating} onOpenChange={setIsCreating}>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                "ui.Routes each request to a model by classifying its complexity. Called like any other model, so clients keep using a single model name.",
              )}
            </DialogDescription>
          </DialogHeader>
          <AddAutoRouterTab
