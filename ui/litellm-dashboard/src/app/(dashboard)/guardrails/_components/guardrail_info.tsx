...
const GuardrailInfoView: React.FC<GuardrailInfoProps> = ({ guardrailId, onClose, accessToken, isAdmin }) => {
  ...
  const fetchGuardrailInfo = async () => {
    try {
      ...
      toast.fromError(t("ui.Failed to load guardrail information"));
  ...
  if (!guardrailData) {
    return <div className="p-4">{t("ui.Guardrail not found")}</div>;
  }
        ...
        <div>
          ...
          <TabsContent value="overview" keepMounted>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <Card className="block p-6">
                ...
                <div className="mt-2">
                  ...
                  <p>
                    {t("ui.Last Updated:")} {formatDate(guardrailData.updated_at)}
                  </p>
                </div>
              </Card>
            </div>
              ...
              Object.keys(guardrailData.litellm_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <div className="flex justify-between items-center">
                    <p className="font-medium">{t("ui.PII Protection")}</p>
                    <Badge variant="secondary">
                      {Object.keys(guardrailData.litellm_params.pii_entities_config).length}{" "}
                      {t("ui.PII entities configured")}
                    </Badge>
                  </div>
                </Card>
              )}
            ...
            {guardrailData.litellm_params?.pii_entities_config &&
              Object.keys(guardrailData.litellm_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <p className="mb-4 text-lg font-semibold">{t("ui.PII Entity Configuration")}</p>
                  <div className="border rounded-lg overflow-hidden shadow-xs">
                    <div className="bg-muted px-5 py-3 border-b flex">
                      <p className="flex-1 font-semibold text-foreground">{t("ui.Entity Type")}</p>
                      <p className="flex-1 font-semibold text-foreground">{t("ui.Configuration")}</p>
                    </div>
              ...
              <Card className="block mt-6 p-6">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="font-medium text-lg">{t("ui.Custom Code")}</p>
                  </div>
          ...
          {isAdmin && (
            <TabsContent value="settings" keepMounted>
              <Card className="block p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium">{t("ui.Guardrail Settings")}</h3>
                  {isConfigGuardrail && (
                    <SimpleTooltip content={t("ui.Guardrail is defined in the config file and cannot be edited.")}>
                      <Info
                        role="img"
                        aria-label={t("ui.Config guardrail details")}
                ...
                {isEditing ? (
                    ...
                    <form onSubmit={form.handleSubmit(submitLatest)}>
                      <FieldGroup>
                        ...
                        >
                          {({ ref, value, ...field }) => (
                            <Input
                              ...
                              ref={ref}
                              value={asText(value)}
                              placeholder={t("ui.Enter guardrail name")}
                            />
                          )}
                        </GuardrailField>

                        <GuardrailField control={form.control} name="default_on" label={t("ui.Default On")}>
                        ...
                        {guardrailData.litellm_params?.guardrail === "presidio" && (
                          <>
                            <SectionHeading>{t("ui.PII Protection")}</SectionHeading>
                            <div className="mb-6">
                              {guardrailSettings && (
                                <PiiConfiguration
                        ...
                        {(guardrailData.litellm_params?.guardrail === "tool_permission" ||
                          ...
                          <SectionHeading>{t("ui.Provider Settings")}</SectionHeading>
                        )}
                        ...
                        <SectionHeading>{t("ui.Advanced Settings")}</SectionHeading>
                        <GuardrailField
                          control={form.control}
                          name="guardrail_info"
                          label={t("ui.Guardrail Information")}
                        >
                          {({ ref, value, ...field }) => (
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="font-medium">{t("ui.Guardrail ID")}</p>
                      <div className="font-mono">{guardrailData.guardrail_id}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("ui.Guardrail Name")}</p>
                    ...
                    <div>
                      <p className="font-medium">{t("ui.Default On")}</p>
                      <Badge variant={guardrailData.litellm_params?.default_on ? "secondary" : "outline"}>
