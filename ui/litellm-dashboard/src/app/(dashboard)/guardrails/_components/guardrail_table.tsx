...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("ui.No guardrails yet")}</div>
      <div className="text-sm text-muted-foreground">{t("ui.Add a guardrail to start filtering requests and responses.")}</div>
    </div>
  );
}
...
const GuardrailTable: React.FC<GuardrailTableProps> = ({
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("ui.Loading guardrails…")}
