  ...
  return (
    <FieldGroup>
      <div className="rounded-md border border-success/20 bg-success/10 px-3.5 py-2.5 text-[13px] text-success">
        {t("ui.After each LLM response, the")} <strong>{t("ui.Judge Model")}</strong>{" "}
        {t(
          "ui.scores it 0–100 against your criteria. If the weighted average falls below the threshold, the response is blocked (or logged).",
        )}
      </div>
      ...
      >
        {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
          <Combobox items={availableModels} value={asText(value) || null} onValueChange={onChange}>
            ...
            />
            <ComboboxContent>
              <ComboboxEmpty>{t("ui.No matching models")}</ComboboxEmpty>
              <ComboboxList>
                {(model: string) => (
        ...
        {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
          <Select items={ON_FAILURE_ITEMS} value={asText(value) || null} onValueChange={onChange}>
            <SelectTrigger id={id} aria-invalid={ariaInvalid} aria-describedby={ariaDescribedBy} className="w-full">
              <SelectValue placeholder={t("ui.Select an action")} />
            </SelectTrigger>
            <SelectContent>
      ...
      <Field>
          ...
          <div key={index} className="mb-2 rounded-md border border-border p-3">
            <div className="flex items-end gap-2">
              ...
              >
                {({ ref, value, ...field }) => (
                  <Input
                    ...
                    ref={ref}
                    value={asText(value)}
                    placeholder={t("ui.Criterion name (e.g. Policy accuracy)")}
                  />
                )}
              </GuardrailField>
              <GuardrailField
                ...
                label={labelWithHint(
                  <span className="text-xs text-muted-foreground">{t("ui.Weight")}</span>,
              ...
              <Button
                variant="ghost"
                size="sm"
                aria-label={t("ui.Remove criterion")}
            ...
            >
              {({ ref, value, ...field }) => (
                <Input
                  ...
                  ref={ref}
                  value={asText(value)}
                  placeholder={t("ui.What should the judge check for this criterion?")}
                />
              )}
            </GuardrailField>
          </div>
        ...
        {criteria.length > 0 && (
          <div className={`mt-1.5 text-xs ${weightOk ? "text-success" : "text-warning"}`}>
            {t("ui.Weights total:")} {weightTotal}%{weightOk ? " ✓" : " — must add up to 100%"}
          </div>
        )}
      </Field>
    </FieldGroup>
