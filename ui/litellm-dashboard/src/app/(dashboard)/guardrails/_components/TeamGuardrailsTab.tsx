    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span>
              {t("ui.Model:")} <span className="font-medium text-foreground">{g.model}</span>
            </span>
            <span>
              {t("ui.Submitted:")} <span className="font-medium text-foreground">{g.submittedAt}</span>
            </span>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground whitespace-nowrap">{t("ui.Forward API Key")}</span>
      ...
      <div className="mt-3 pt-3 border-t border-border">
        ...
        {isHeadersExpanded && (
          <div className="mt-2">
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">{t("ui.No static headers configured.")}</p>
            ) : (
    ...
    <div className="w-96 shrink-0 bg-card overflow-auto">
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("ui.Submitted by")} {g.submittedBy} on {g.submittedAt}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-4">
          ...
          <div className="border border-info/15 bg-info/10 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-info">{t("ui.Forward LiteLLM API Key")}</span>
              </div>
              <Toggle enabled={g.forwardKey} onToggle={onToggleForwardKey} disabled={!isAdmin} />
            </div>
            <p className="text-xs text-info leading-relaxed">
              ...
              {t(
                "ui.header to your guardrail endpoint. This allows your guardrail to authenticate model calls using the original caller's credentials.",
              )}
            </p>
          </div>
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">{t("ui.Static headers")}</span>
              {g.customHeaders.length > 0 && (
            ...
            <p className="text-xs text-muted-foreground mb-2">{t("ui.Sent with every request to the guardrail.")}</p>
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">{t("ui.No static headers configured.")}</p>
            ) : (
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">{t("ui.Forward client headers")}</span>
            ...
            {g.extraHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">
                {t("ui.No forward client headers configured.")}
              </p>
            ) : (
              <ul className="list-none space-y-1 mb-2">
          ...
          <div className="flex items-start gap-2 bg-muted border border-border rounded-lg p-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              {t(
                "ui.This guardrail runs on a separate instance. It receives the user request and forwards the result to the next step in the pipeline. See",
              )}{" "}
              <a
...
export function TeamGuardrailsTab({ accessToken }: TeamGuardrailsTabProps) {
  ...
  const handleSubmitGuardrail = submitForm.handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.success(t("ui.Guardrail submitted for review"));
  ...
  async function toggleForwardKey(id: string) {
    ...
    try {
      ...
      toast.fromError(t("ui.Failed to update forward API key"));
    }
  }
  ...
  async function updateCustomHeaders(id: string, customHeaders: { key: string; value: string }[]) {
    ...
    try {
      ...
      toast.success(t("ui.Static headers updated"));
    } catch {
      toast.fromError(t("ui.Failed to update static headers"));
    }
  }
  ...
  async function updateExtraHeaders(id: string, extraHeaders: string[]) {
    ...
    try {
      ...
      toast.success(t("ui.Forward client headers updated"));
    } catch {
      toast.fromError(t("ui.Failed to update forward client headers"));
    }
  }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      toast.success(t("ui.Guardrail approved"));
    } catch {
      toast.fromError(t("ui.Failed to approve guardrail"));
    }
  }
  ...
  async function handleReject(id: string) {
    ...
    try {
      ...
      if (selectedId === id) setSelectedId(null);
      await fetchSubmissions();
      toast.success(t("ui.Guardrail rejected"));
    } catch {
      toast.fromError(t("ui.Failed to reject guardrail"));
    }
  }
    ...
    <div className="flex h-full">
      <div className={`flex-1 min-w-0 p-6 overflow-auto ${selected ? "border-r border-border" : ""}`}>
        <div className="grid grid-cols-4 gap-4 mb-6">
          <StatCard label={t("ui.Total Submitted")} value={totalCount} color="text-foreground" />
          <StatCard label={t("ui.Pending Review")} value={pendingCount} color="text-warning" />
          <StatCard label={t("ui.Active")} value={activeCount} color="text-success" />
          <StatCard label={t("ui.Rejected")} value={rejectedCount} color="text-destructive" />
        </div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <select
            aria-label={t("ui.Filter by status")}
            value={statusFilter}
          ...
          >
            <option value="all">{t("ui.All Status")}</option>
        ...
        <div className="space-y-3">
          {isLoading && (
            <div className="text-center py-12 text-muted-foreground text-sm">{t("ui.Loading submissions…")}</div>
          )}
          {error && <div className="text-center py-12 text-destructive text-sm">{error}</div>}
          {!isLoading && !error && filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground text-sm">
              {t("ui.No guardrails match your filters.")}
            </div>
          )}
          {!isLoading &&
            !error &&
      ...
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("ui.Submit Guardrail for Review")}</DialogTitle>
          </DialogHeader>
          ...
          <TooltipProvider>
            <form onSubmit={handleSubmitGuardrail}>
              <FieldGroup>
                <FormField control={submitForm.control} name="team_id" label={t("ui.Team")}>
                  {({ id, value, onChange }) => <TeamDropdown id={id} value={value} onChange={onChange} />}
                </FormField>
                <FormField control={submitForm.control} name="guardrail_name" label={t("ui.Guardrail Name")}>
                  {({ ref, ...field }) => <Input {...field} ref={ref} placeholder={t("ui.e.g. pii-detection")} />}
                </FormField>
                <FormField control={submitForm.control} name="mode" label={t("ui.Mode")}>
                ...
                <FormField control={submitForm.control} name="api_base" label={t("ui.API Base URL")}>
                  {({ ref, ...field }) => (
                    <Input
                      {...field}
                      ref={ref}
                ...
                <FormField control={submitForm.control} name="guardrail_info" label={t("ui.Guardrail Info (optional)")}>
                  {({ ref, ...field }) => (
                    <Textarea
                      {...field}
                      ref={ref}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitGuardrail}>{t("ui.Submit for Review")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
