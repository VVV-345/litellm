...
export function GuardrailTestPanel({
  ...
  const handleSubmit = () => {
    if (!inputText.trim()) {
      toast.fromError(t("ui.Please enter text to test"));
      return;
    }
  ...
  const handleCopyInput = async () => {
    ...
    if (success) {
      toast.success(t("ui.Input copied to clipboard"));
    } else {
      toast.fromError(t("ui.Failed to copy input"));
    }
  };
        ...
        <div className="flex items-center space-x-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              {t("ui.Test")} {guardrailNames.length > 1 ? "guardrails" : "guardrail"} and compare results
            </p>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="mt-1 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">
                {t("ui.Press")}{" "}
                <kbd className="rounded-sm border border-border bg-muted px-1 py-0.5 text-xs">Enter</kbd> to submit •{" "}
                <kbd className="rounded-sm border border-border bg-muted px-1 py-0.5 text-xs">Shift+Enter</kbd>{" "}
                {t("ui.for new line")}
              </span>
              <span className="text-xs text-muted-foreground">
                {t("ui.Characters:")} {inputText.length}
              </span>
            </div>
          </div>
