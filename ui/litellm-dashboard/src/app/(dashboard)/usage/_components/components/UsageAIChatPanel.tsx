    ...
    try {
      await usageAiChatStream(
        ...
        (errorMsg: string) => {
          ...
          setMessages((prev) => [
            ...prev,
            { role: "assistant", content: t("ui.Error: {{message}}", { message: errorMsg }) },
          ]);
          setStreamingContent("");
        },
        (status: string) => {
      ...
      const errorMsg = error?.message || t("ui.Failed to get response. Please try again.");
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: t("ui.Error: {{message}}", { message: errorMsg }) },
      ]);
    ...
    >
      ...
      <div className="px-5 pt-5 pb-3 border-b border-border shrink-0">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-foreground">{t("ui.Ask AI")}</h3>
          </div>
          <button
            onClick={handleClose}
        ...
        <p className="text-xs text-muted-foreground">{t("ui.Ask about your spend, models, keys, and trends")}</p>
      </div>
      ...
      <div className="px-5 py-3 border-b border-border shrink-0">
        ...
        >
          <ComboboxInput
            className="w-full"
            placeholder={t("ui.Select a model (optional, defaults to gpt-4o-mini)")}
            aria-label={t("ui.Select a model (optional, defaults to gpt-4o-mini)")}
            aria-busy={isLoadingModels}
            showClear={selectedModel !== undefined}
          />
          <ComboboxContent>
            <ComboboxEmpty>{isLoadingModels ? t("ui.Loading models…") : t("ui.No models found")}</ComboboxEmpty>
            <ComboboxList>
              {(model: string) => (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-muted">
        {messages.length === 0 && !streamingContent && !isLoading && (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            ...
            <p className="text-sm font-medium">{t("ui.Ask a question about your usage")}</p>
            <p className="text-xs mt-1">{t('ui.e.g. "Which model costs me the most?"')}</p>
          </div>
        )}
        ...
        {isLoading && !streamingContent && (
          <div className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground">
            <UiLoadingSpinner className="size-3.5" />
            <span className="italic">{statusMessage || t("ui.Thinking...")}</span>
          </div>
        )}
      ...
      <div className="px-4 py-3 border-t border-border bg-card shrink-0">
        <div className="flex gap-2">
          <Textarea
            ...
            placeholder={t("ui.Ask about your usage...")}
            rows={1}
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <span className="text-xs text-muted-foreground">{t("ui.Enter to send")}</span>
        </div>
      </div>
    </div>
  );
