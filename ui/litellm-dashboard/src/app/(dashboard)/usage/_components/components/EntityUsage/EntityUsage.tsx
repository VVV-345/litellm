...
const EntityUsage: React.FC<EntityUsageProps> = ({
    ...
    <div className="grid grid-cols-2 gap-2 w-full">
        ...
        <ShadcnCard>
          <CardContent>
            <h3 className="text-lg font-medium text-foreground">
              {capitalizedEntityLabel} {t("ui.Spend Overview")}
            </h3>
                ...
                return (
                  <div className="bg-card p-4 shadow-lg rounded-lg border">
                    ...
                    {showFlatCost ? (
                      <>
                        <p className="text-info">
                          {t("ui.Request cost: $")}
                          {formatNumberWithCommas(requestSpend, 2)}
                        </p>
                        <p className="text-violet-500">
                          {t("ui.Flat cost: $")}
                          {formatNumberWithCommas(flatCost, 2)}
                        </p>
                        <p className="font-semibold">
                          {t("ui.Total cost: $")}
                          {formatNumberWithCommas(requestSpend + flatCost, 2)}
                        </p>
                      </>
                    ) : (
                      <p className="text-info">
                    ...
                    <p className="text-muted-foreground">
                      {t("ui.Total Requests:")} {data.metrics.api_requests}
                    </p>
                    <p className="text-muted-foreground">
                    ...
                    <p className="text-muted-foreground">
                      {t("ui.Total Tokens:")} {data.metrics.total_tokens}
                    </p>
                    <p className="text-muted-foreground">
                      {t("ui.Total")} {capitalizedEntityLabel}s: {entityCount}
                    </p>
                    <div className="mt-2 border-t pt-2">
                      <p className="font-semibold">
                        {t("ui.Spend by")} {capitalizedEntityLabel}:
                      </p>
        ...
        <ShadcnCard>
          <CardContent className="flex flex-col space-y-4">
            <div className="flex flex-col space-y-2">
              <h3 className="text-lg font-medium text-foreground">
                {t("ui.Spend Per")} {capitalizedEntityLabel}
              </h3>
              <p className="text-xs text-muted-foreground">{t("ui.Showing Top 5 by Spend")}</p>
              <div className="flex items-center text-sm text-muted-foreground">
                <span>
                  {t("ui.Get Started by Tracking cost per")} {capitalizedEntityLabel}{" "}
                </span>
                <a
            ...
            <div className="grid grid-cols-2 gap-6">
                  ...
                  customTooltip={({ payload, active }) => {
                    ...
                    return (
                      <div className="bg-card p-4 shadow-lg rounded-lg border">
                        ...
                        <p className="text-muted-foreground">
                          {t("ui.Requests:")} {data.metrics.api_requests.toLocaleString()}
                        </p>
                        <p className="text-success">
                          {t("ui.Successful:")} {data.metrics.successful_requests.toLocaleString()}
                        </p>
                        <p className="text-destructive">
                          {t("ui.Failed:")} {data.metrics.failed_requests.toLocaleString()}
                        </p>
                        <p className="text-muted-foreground">
                          {t("ui.Tokens:")} {data.metrics.total_tokens.toLocaleString()}
                        </p>
                      </div>
                    );
                  }}
              ...
              <div>
                <DataTable
                  ...
                  maxBodyHeight={208}
                  noDataMessage={t("ui.No spend data")}
                  size="compact"
                />
              </div>
            </div>
        ...
        <ShadcnCard>
          <CardContent>
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-medium text-foreground">
                {entityType === "agent" ? t("ui.Top Agents") : modelViewTitle}
              </h3>
      ...
      {showAgentBreakdown && (
        <div className="col-span-2">
          <ShadcnCard>
            <CardContent>
              <h3 className="text-lg font-medium text-foreground">{t("ui.Top Agents Driving Spend")}</h3>
              <TopModelView
      ...
      <div className="col-span-2">
        <ShadcnCard>
          <CardContent className="flex flex-col space-y-4">
            <h3 className="text-lg font-medium text-foreground">{t("ui.Provider Usage")}</h3>
  ...
  const tabs: readonly { key: string; label: string; content: ReactNode }[] = [
    { key: "cost", label: t("ui.Cost"), content: costPanel },
    {
      key: "models",
      label: entityType === "agent" ? t("ui.Request / Token Consumption") : t("ui.Model Activity"),
      content: (
        <>
