...
const CostTrackingSettings: React.FC<CostTrackingSettingsProps> = ({ userID, userRole, accessToken }) => {
  ...
  const docsLinks = [
    { label: t("ui.Custom pricing for models"), href: "https://docs.litellm.ai/docs/proxy/custom_pricing" },
    { label: t("ui.Spend tracking"), href: "https://docs.litellm.ai/docs/proxy/cost_tracking" },
  ];
  ...
  const removalCopy =
    pendingRemoval?.kind === "discount"
      ? { title: t("ui.Remove Provider Discount"), item: t("ui.Discount") }
      : { title: t("ui.Remove Provider Margin"), item: t("ui.Margin") };
    ...
    <div className="w-full p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2">
            <p className="text-xl font-medium text-foreground">{t("ui.Cost Tracking Settings")}</p>
            <DocsMenu items={docsLinks} />
          </div>
          <p className="text-muted-foreground mt-1">
            {t("ui.Configure cost discounts and margins for different LLM providers. Changes are saved automatically.")}
          </p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-lg shadow-sm w-full max-w-full space-y-4">
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader
              title={t("ui.Provider Discounts")}
              description={t("ui.Apply percentage-based discounts to reduce costs for specific providers")}
            />
                ...
                <TabsContent value="discounts" keepMounted>
                  <div className="p-6">
                    <div className="flex justify-end mb-4">
                      <Button onClick={() => setIsModalVisible(true)}>{t("ui.+ Add Provider Discount")}</Button>
                    </div>
                    {isFetching ? (
                      <div className="py-12 text-center">
                        <p className="text-muted-foreground">{t("ui.Loading configuration...")}</p>
                      </div>
                      ...
                      <div className="py-16 px-6 text-center">
                        ...
                        <p className="text-foreground font-medium mb-2">{t("ui.No provider discounts configured")}</p>
                        <p className="text-muted-foreground text-sm">
                          {t('ui.Click "Add Provider Discount" to get started')}
                        </p>
                      </div>
                    )}
                  </div>
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader
              title={t("ui.Fee/Price Margin")}
              description={t("ui.Add fees or margins to LLM costs for internal billing and cost recovery")}
            />
            <CollapsibleContent className="px-0">
              <div className="p-6">
                <div className="flex justify-end mb-4">
                  <Button onClick={() => setIsMarginModalVisible(true)}>{t("ui.+ Add Provider Margin")}</Button>
                </div>
                {isFetching ? (
                  <div className="py-12 text-center">
                    <p className="text-muted-foreground">{t("ui.Loading configuration...")}</p>
                  ...
                  <div className="py-16 px-6 text-center">
                    ...
                    <p className="text-foreground font-medium mb-2">{t("ui.No provider margins configured")}</p>
                    <p className="text-muted-foreground text-sm">
                      {t('ui.Click "Add Provider Margin" to get started')}
                    </p>
                  </div>
                )}
              </div>
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader
              title={t("ui.Block Unpriced Models")}
              description={t(
                "ui.Reject requests for models that have no pricing in the cost map instead of logging them as $0 spend",
              )}
            />
            <CollapsibleContent className="px-0">
              <div className="p-6">
                <div className="flex items-center justify-between">
                  <div className="pr-6">
                    <p className="text-foreground font-medium">{t("ui.Block requests for models without pricing")}</p>
                    <p className="text-sm text-muted-foreground mt-1">
                      {t(
                        "ui.When enabled, a request whose resolved model has no cost mapping is rejected with a 403 so an admin can add pricing for it. Off by default",
                      )}
                    </p>
                  </div>
                  <Switch
        ...
        <Collapsible defaultOpen={true} className="rounded-lg border">
          <SectionHeader
            title={t("ui.Pricing Calculator")}
            description={t("ui.Estimate LLM costs based on expected token usage and request volume")}
          />
          <CollapsibleContent className="px-0">
            <div className="p-6">
      ...
      {pendingRemoval && (
        <AlertDialog open onOpenChange={(open) => !open && !isRemoving && setPendingRemoval(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{removalCopy.title}</AlertDialogTitle>
              <AlertDialogDescription>
                {t("ui.Are you sure you want to remove {{item}} for {{name}}?", {
                  item: removalCopy.item,
                  name: pendingRemoval.displayName,
                })}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={isRemoving}>{t("ui.Cancel")}</AlertDialogCancel>
              <Button variant="destructive" onClick={handleConfirmRemoval} disabled={isRemoving}>
                {isRemoving ? t("ui.Removing…") : t("ui.Remove")}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      ...
      <Dialog open={isMarginModalVisible} onOpenChange={(open) => !open && handleMarginModalCancel()}>
        <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
          <DialogHeader>
            <div className="flex items-center space-x-3 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-semibold text-foreground">{t("ui.Add Provider Margin")}</DialogTitle>
            </div>
          </DialogHeader>
          <div className="mt-6">
            <p className="text-sm text-muted-foreground mb-6">
