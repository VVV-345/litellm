...
const SingleModelBreakdown: React.FC<{
  ...
  const periodLabel = t(timePeriod === "day" ? "ui.Daily" : "ui.Monthly");
    ...
    <div className="space-y-3 bg-muted p-4 rounded-lg">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("ui.Total/Request")}</p>
          <p className="text-base font-semibold text-info break-words">{formatCost(result.cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("ui.Input Cost")}</p>
          <p className="text-sm break-words">{formatCost(result.input_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("ui.Output Cost")}</p>
          <p className="text-sm break-words">{formatCost(result.output_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("ui.Margin Fee")}</p>
        ...
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-border">
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground block">
              {t("ui.{{period}} Total ({{requests}} req)", {
                period: periodLabel,
      ...
      {(result.input_cost_per_token || result.output_cost_per_token) && (
        <div className="text-xs text-muted-foreground pt-2 border-t border-border">
          ...
          {result.input_cost_per_token && (
            <span>
              {t("ui.Input $")}
              {formatNumberWithCommas(result.input_cost_per_token * 1_000_000, 2)}/1M
            </span>
          )}
          {result.input_cost_per_token && result.output_cost_per_token && " | "}
          {result.output_cost_per_token && (
            <span>
              {t("ui.Output $")}
              {formatNumberWithCommas(result.output_cost_per_token * 1_000_000, 2)}/1M
            </span>
          )}
        </div>
      )}
  ...
  if (!hasAnyResult && !isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center border border-dashed border-border rounded-lg bg-muted">
        <p className="text-muted-foreground">{t("ui.Select models above to see cost estimates")}</p>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-muted-foreground block mt-2">{t("ui.Calculating costs...")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-base font-semibold text-foreground">{t("ui.Cost Estimates")}</p>
        ...
        {errorEntries.map((e) => (
          ...
          >
            <span className="font-medium">{e.entry.model || t("ui.Unknown model")}: </span>
            {e.error}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <Card size="sm" className="px-4 bg-linear-to-r from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-950">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">{t("ui.Total Per Request")}</span>
        ...
        {hasMargin && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 mt-3 pt-3 border-t border-border">
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{t("ui.Margin Fee/Request")}</div>
      ...
      {summaryData.length > 0 && (
        <Table className="border border-border rounded-lg">
          <TableHeader>
            <TableRow>
              <TableHead>{t("ui.Model")}</TableHead>
              <TableHead className="text-right">{t("ui.Per Request")}</TableHead>
              <TableHead className="text-right">{t("ui.Margin Fee")}</TableHead>
              <TableHead className="text-right">{periodLabel}</TableHead>
              <TableHead className="w-10">
                <span className="sr-only">{t("ui.Cost breakdown")}</span>
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                      ...
                      <div className="flex min-w-0 flex-col gap-1">
                        ...
                        {record.hasZeroCost && !record.error && (
                          <div className="text-xs text-warning bg-warning/10 px-2 py-1 rounded-sm">
                            {t("ui.⚠️ No pricing data found for this model. Set base_model in config.")}
                          </div>
                        )}
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      {!record.error && (
                        <Button
                          ...
                          aria-label={t(
                            isExpanded ? "ui.Hide cost breakdown for {{model}}" : "ui.Show cost breakdown for {{model}}",
                            { model: record.model },
                          )}
