    ...
    <div className="mb-3 p-3 bg-warning/10 border border-warning/20 rounded-lg">
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <p className="text-sm text-warning font-medium mb-1">{t("ui.Please fill in all template variables above")}</p>
          <p className="text-xs text-warning">
            {t("ui.Missing:")} {missingVariables.map((varName) => `{{${varName}}}`).join(", ")}
          </p>
        </div>
      </div>
    </div>
