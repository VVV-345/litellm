    ...
    <Card className="overflow-hidden py-0">
      <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <div className="flex flex-col justify-center border-t p-6 md:border-t-0 md:border-l">
          <SpendRow label={t("ui.Actual auto-router spend")} value={usd(stats.spend)} />
          <Separator />
          <SpendRow label={t("ui.Estimated spend at highest-tier model")} value={usd(stats.baseline_spend)} />
        </div>
      </div>
    </Card>
  );
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metric
          label={t("ui.Avg saved per session")}
        ...
        <Metric
          label={t("ui.Avg tokens per session")}
      ...
      <p className="text-xs text-muted-foreground">
        {t(
          "ui.Compares your actual routed spend with the estimated cost of using only the most expensive model configured in the auto-router. It accounts for both the cache savings from staying on one model and the added cache costs from switching models. The range counts whole sessions that overlap it, so totals can differ slightly from the Overall tab, which buckets savings by UTC day.",
        )}
      </p>
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            {t("ui.every turn falls in exactly one bucket, by what the router did")}
          </p>
        </div>
