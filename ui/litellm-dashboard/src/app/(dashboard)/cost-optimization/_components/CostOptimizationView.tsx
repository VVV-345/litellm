    ...
    <div className="w-full space-y-6 p-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold text-foreground">{t("ui.Cost Optimization")}</h1>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">
          {t(
            "ui.Track and configure the mechanisms that save you money: prompt compression and prompt caching. Auto routers live under Models + Endpoints, on the Auto-Routers tab",
          )}
        </p>
      </div>
      ...
      >
        ...
        <p className="font-medium text-foreground">{t("ui.This is an experimental dashboard")}</p>
        <p className="col-start-2 text-sm text-muted-foreground">
          {t("ui.Have feedback? Join the discussion")}{" "}
          <a
