...
const PromptCompressionTab: React.FC<PromptCompressionTabProps> = ({ accessToken }) => {
  ...
  const loadGuardrails = useCallback(() => {
    ...
    getGuardrailsList(accessToken)
      ...
      .catch((error) => {
        ...
        toast.fromError(t("ui.Failed to load compression guardrails"));
      })
  ...
  const handleAdd = async (values: CompressionFormValues) => {
    ...
    try {
      ...
      toast.success(t("ui.Compression guardrail created"));
      form.reset(EMPTY_VALUES);
      await loadGuardrails();
    } catch (error) {
      console.error("Failed to create compression guardrail:", error);
      toast.fromError(t("ui.Failed to create compression guardrail"));
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <CardContent>
          <TooltipProvider>
            <form onSubmit={form.handleSubmit(handleAdd)} noValidate>
              <FieldGroup>
                <FormField control={form.control} name="name" label={t("ui.Name")}>
                ...
                <FormField
                  ...
                  description={t("ui.The URL where your Headroom compression service is hosted")}
                >
                  {({ ref, ...field }) => <Input {...field} ref={ref} placeholder="https://your-headroom-endpoint" />}
                </FormField>
                <FormField control={form.control} name="defaultOn" label={t("ui.Apply to all requests")}>
