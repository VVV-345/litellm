...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  const renderEntitlementsStep = () => (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">{ui("Configure which models, agents, and MCP tools this agent is allowed to use. Leave fields empty to allow all (subject to key/team permissions).")}</p>

      <Form.Item
        label={<span className="text-sm font-medium text-gray-700">{ui("Allowed Models")}</span>}
        name="entitlement_models"
        tooltip={ui("Restrict which models this agent can call. Leave empty to allow all.")}
      >
        <Select
          mode="tags"
          style={{ width: "100%" }}
          placeholder={loadingModels ? ui("Loading models...") : ui("Select models (leave empty for all)")}
          tokenSeparators={[","]}
      ...
      <Form.Item
        label={<span className="text-sm font-medium text-gray-700">{ui("Allowed Agents (Sub-Agents)")}</span>}
        name="entitlement_agents"
        tooltip={ui("Restrict which other agents this agent can invoke as sub-agents. Leave empty to allow all.")}
      >
        <Select
          mode="multiple"
          style={{ width: "100%" }}
          placeholder={loadingAgents ? ui("Loading agents...") : ui("Select agents (leave empty for all)")}
      ...
      <Form.Item
        label={
          <span>
            {ui("Allowed MCP Servers")}{" "}
            <InfoCircleOutlined
              title={ui("Select which MCP servers or access groups this agent can access")}
      ...
      >
        <MCPServerSelector
          ...
          placeholder={ui("Select MCP servers or access groups (optional)")}
        />
      </Form.Item>
  ...
  const renderObservabilityStep = () => (
    <div className="space-y-6">
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{ui("Tracing")}</h4>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">
                {ui("Require x-litellm-trace-id on calls TO this agent")}
              </span>
              <p className="text-xs text-gray-500 mt-1">
                {ui("Only accept this agent being invoked with a trace-id (e.g. when used as a sub-agent).")}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">
                {ui("Require x-litellm-trace-id on calls BY this agent")}
              </span>
              <p className="text-xs text-gray-500 mt-1">
                {ui("Requires LLM/MCP calls made by this agent to include x-litellm-trace-id for session tracking.")}
              </p>
            </div>
            <Switch
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{ui("Budgets & Rate Limits")}</h4>
        <div className="space-y-4">
          ...
          <div className="text-sm font-medium text-gray-700">{ui("Session Budgets")}</div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-gray-600 block mb-1">{ui("Max Iterations")}</label>
              <InputNumber
                className="w-full"
                min={1}
                placeholder={ui("e.g. 25")}
                disabled={!requireTraceIdOutbound}
                value={maxIterations}
                onChange={(val) => setMaxIterations(val)}
              />
              <p className="text-xs text-gray-400 mt-1">{ui("Hard cap on LLM calls per session")}</p>
            </div>
            <div>
              <label className="text-sm text-gray-600 block mb-1">{ui("Max Budget Per Session ($)")}</label>
              <InputNumber
                className="w-full"
                min={0.01}
                step={0.5}
                placeholder={ui("e.g. 5.00")}
                disabled={!requireTraceIdOutbound}
                value={maxBudgetPerSession}
                onChange={(val) => setMaxBudgetPerSession(val)}
              />
              <p className="text-xs text-gray-400 mt-1">{ui("Max spend per trace before returning 429")}</p>
            </div>
          </div>

          <Divider className="my-2" />

          <div className="text-sm font-medium text-gray-700">{ui("Agent Rate Limits")}</div>
          <p className="text-xs text-gray-500">{ui("Global rate limits applied across all callers of this agent.")}</p>
          <div className="grid grid-cols-2 gap-4">
            <Form.Item label={ui("TPM Limit")} name="tpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={ui("e.g. 100000")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
            <Form.Item label={ui("RPM Limit")} name="rpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={ui("e.g. 100")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
          </div>

          <div className="text-sm font-medium text-gray-700 mt-4">{ui("Per-Session Rate Limits")}</div>
          <p className="text-xs text-gray-500">
            {ui("Rate limits per session (x-litellm-trace-id). Each session gets its own counters.")}
          </p>
          <div className="grid grid-cols-2 gap-4">
            <Form.Item label={ui("Session TPM Limit")} name="session_tpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={ui("e.g. 10000")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
            <Form.Item label={ui("Session RPM Limit")} name="session_rpm_limit" className="mb-0">
              <InputNumber className="w-full" min={0} placeholder={ui("e.g. 20")} disabled={!requireTraceIdOutbound} />
            </Form.Item>
          </div>
        </div>
      </div>
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-3">{ui("Guardrails")}</h4>
        <p className="text-xs text-gray-500 mb-3">
          {ui("Apply guardrails to this agent. Selected guardrails will run on all calls made by this agent.")}
        </p>
  ...
  const renderConfigureStep = () => (
    <>
      <Form.Item
        label={<span className="text-sm font-medium text-gray-700">{ui("Agent Type")}</span>}
        required
        tooltip={ui("Select the type of agent you want to create")}
      >
        <Select
          value={agentType}
          ...
          dropdownRender={(menu) => (
            <>
              ...
              <div className="px-2 py-1">
                <div className="text-xs text-gray-400 font-medium mb-1 uppercase tracking-wide px-2">{ui("Not listed?")}</div>
                <div
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-amber-700">{ui("Custom / Other")}</span>
                      <Tag color="orange" style={{ fontSize: 10, padding: "0 4px" }}>
                        GENERIC
                      </Tag>
                    </div>
                    <div className="text-xs text-amber-600">
                      {ui("For agents that don't follow a standard protocol — just needs a virtual key")}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {agentType === CUSTOM_AGENT_TYPE ? (
          <div className="space-y-4">
            <Form.Item
              label={ui("Agent Name")}
              name="agent_name"
              rules={[{ required: true, message: ui("Please enter an agent name") }]}
            >
              <Input placeholder={ui("e.g. my-custom-agent")} />
            </Form.Item>
            <Form.Item label={ui("Description")} name="description">
              <Input.TextArea placeholder={ui("Describe what this agent does…")} rows={3} />
            </Form.Item>
          </div>
        ) : agentType === "a2a" ? (
  ...
  const renderAssignKeyStep = () => {
    ...
    return (
      <div>
        ...
        <Form.Item
          label={<span className="text-sm font-medium text-gray-700">{ui("Assign to Team")}</span>}
          name="team_id"
          tooltip={ui("Optionally assign this agent to a team. The agent and its key will belong to the selected team.")}
        >
          <TeamDropdown />
        </Form.Item>
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-900">{ui("Create a new key for this agent")}</span>
                  </div>
                  <p className="text-sm text-gray-500 mt-1">{ui("A dedicated key scoped to this agent.")}</p>
                  {keyAssignOption === "create_new" && (
                    <div className="mt-3 space-y-3" onClick={(e) => e.stopPropagation()}>
                      <div>
                        <label className="text-sm text-gray-600 block mb-1">{ui("Key Name")}</label>
                        <Input
                          value={newKeyName}
                          onChange={(e) => setNewKeyName(e.target.value)}
                          placeholder={ui("e.g. my-agent-key")}
                        />
                      </div>
                    </div>
                  )}
              ...
              <Tag color="green">{ui("Recommended")}</Tag>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-900">{ui("Assign an existing key")}</span>
                </div>
                <p className="text-sm text-gray-500 mt-1">{ui("Re-assign a key you already have to this agent.")}</p>
                {keyAssignOption === "existing_key" && (
                  <div className="mt-3" onClick={(e) => e.stopPropagation()}>
                    <Select
                      showSearch
                      style={{ width: "100%" }}
                      placeholder={ui("Search by key name…")}
                      loading={loadingKeys}
        ...
        <div className="text-center mt-4">
          ...
          >
            {ui("Skip for now — I'll assign a key later")}
          </button>
        </div>
      </div>
    );
  ...
  const renderReadyStep = () => (
    <div className="text-center py-6">
      ...
      <h3 className="text-xl font-semibold text-gray-900 mb-2">{ui("Agent Created!")}</h3>
      ...
      {assignedKeyAlias && (
        <p className="text-sm text-gray-600 mt-2">
          {ui("Key")} <span className="font-medium">{assignedKeyAlias}</span> {ui("has been assigned to this agent.")}
        </p>
      )}
      {!createdKeyValue && !assignedKeyAlias && keyAssignOption === "skip" && (
        <p className="text-sm text-gray-500 mt-2">{ui("No key assigned. You can create one from the Virtual Keys page.")}</p>
      )}
    </div>
  );
      ...
      title={
        <div className="flex items-center space-x-3 pb-4 border-b border-gray-100">
          {selectedLogo && currentStep < 1 && (
            ...
            <h2 className="text-xl font-semibold text-gray-900">{ui("Add New Agent")}</h2>
        </div>
      }
      open={visible}
    ...
    >
      <div className="mt-4">
        ...
        <Steps current={currentStep} size="small" className="mb-8">
          <Step title={ui("Configure")} />
          <Step title={ui("Entitlements")} />
          <Step title={ui("Governance")} />
          <Step title={ui("Agent Management")} />
          <Step title={ui("Ready")} />
        </Steps>
          ...
          <div className="flex gap-3">
            ...
            {currentStep === 3 && (
              <Button variant="primary" loading={isSubmitting} onClick={handleCreateAgent}>
                {isSubmitting ? ui("Creating...") : ui("Create Agent →")}
              </Button>
            )}
            {currentStep === 4 && (
