...
const AgentsPanel: React.FC<AgentsPanelProps> = ({ accessToken, userRole, teams }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t('ui.Agent "{{name}}" deleted successfully', { name: agentToDelete.name }));
  ...
  return (
    <div className="w-full mx-auto flex-auto overflow-y-auto m-8 p-2">
      <div className="flex flex-col gap-2 mb-4">
        <h1 className="text-2xl font-bold">{t("ui.Agents")}</h1>
        <p className="text-sm text-muted-foreground">
          {t(
            "ui.List of A2A-spec agents that are available to be used in your organization. Go to AI Hub, to make agents public.",
          )}
        </p>
        <Alert className="mb-3">
          <Info />
          <AlertTitle>{t("ui.Why do agents need keys?")}</AlertTitle>
          <AlertDescription>
            {t(
              "ui.Keys scope access to an agent and allow it to call MCP tools. Assign a key when creating an agent or from the Virtual Keys page.",
            )}
          </AlertDescription>
        </Alert>
        {isAdmin && (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t("ui.Are you sure you want to delete agent: {{name}}? This action cannot be undone.", {
