...
import type { AccountPoolCreateRequest } from "../utils/AccountPoolTypes";
...
describe("createAccountPoolEnvironment", () => {
  ...
  it("retries a gateway failure once with the same idempotency key", async () => {
    ...
    postMock
      .mockRejectedValueOnce(new ApiError("Bad Gateway", 502, undefined))
  ...
  it("retries deletion safely after a gateway failure", async () => {
    deleteMock.mockRejectedValueOnce(new ApiError("Gateway Timeout", 504, undefined)).mockResolvedValueOnce(undefined);
