...
type AccountPoolQuotaRefreshResult = components["schemas"]["AccountPoolQuotaRefreshResult"];
...
type CodexReviewPackage = components["schemas"]["CodexReviewPackage"];
type AccountPoolPluginRuntimeResponse = Record<string, unknown>;
type AccountPoolCredentialDeleteRequest = {
  version: number;
  credential_index: number;
};
interface AccountPoolDashboardStats {
  summary: ErrorStats;
  cards: ErrorStats[];
