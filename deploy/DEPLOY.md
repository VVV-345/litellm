...
号池请求按实际上游尝试同步到标准 LiteLLM Usage 与 SpendLogs，保存请求时的模型价格快照。缓存采用对应协议的 Token 定义，WebSocket 多轮逐轮计价。成本为上游返回值或模型价格估算，不等同于订阅账号的实际账单。缺少价格或用量时，号池显示未知；标准 Spend 的数值字段使用 0 占位并在元数据标记 `cost_known=false`，日志表显示未知，汇总金额仅包含已知部分。同步在同一数据库事务中去重写入，失败最多重试两次并在日常日志显示状态，暂不提供故障期间记录的自动补账
